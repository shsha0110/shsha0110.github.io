...
description: "정확한 MAP 디코딩이 왜 불가능한지에서 출발해 빔 탐색과 샘플링 전략을 구현하고, 자기회귀 생성의 중복 계산을 KV 캐시로 걷어내 속도를 실측합니다."
author: "유성현"
...
> 본 포스트는 서울대학교 컴퓨터공학부 **Hyun Oh Song** 교수님의 *Introduction to Deep Learning* (M2177.0043) 12강 실습 세션(*Decoding algorithms*)의 노트북을 바탕으로, 코드와 그 배경이 되는 논리를 함께 따라갈 수 있도록 재구성한 것입니다.

## 한 줄 요약 (TL;DR)

> **"학습이 끝난 모델에서 문장을 뽑아내는 일에도 알고리즘이 필요하고, 그 알고리즘의 비용은 KV 캐시로 결정적으로 달라집니다."**
>
> 모델을 다 학습시켰다고 해서 좋은 문장이 저절로 나오지는 않습니다. 이번 실습은 추론 시점의 두 가지 질문을 다룹니다. 첫째, **무엇을 뽑을 것인가.** 이상적으로는 확률이 가장 높은 시퀀스 전체를 찾고 싶지만, 후보 시퀀스의 수가 어휘 크기의 길이 제곱으로 늘어나 정확한 MAP 디코딩은 계산 불가능합니다. 그래서 매 시점 최선만 고르는 그리디, 상위 $B$ 개 부분 시퀀스를 함께 끌고 가는 **빔 탐색**, 그리고 다양성을 위해 확률 분포에서 뽑되 꼬리를 잘라 내는 **Top-K · Top-p 샘플링** 같은 근사가 등장합니다. 로그 확률을 더하는 이유(부동소수점 언더플로 회피)와 빔 폭 · 길이에 따른 복잡도도 직접 측정합니다. 둘째, **어떻게 빠르게 뽑을 것인가.** 자기회귀 생성은 토큰 하나를 늘릴 때마다 앞선 토큰들의 키와 값을 **매번 다시 계산**하는 낭비를 저지릅니다. 이미 계산한 $K, V$ 를 저장해 두는 **KV 캐시**는 이 중복을 없애고, 실측에서 시퀀스가 길어질수록 격차가 급격히 벌어집니다.
...
| 단계 | 내용 | 근거 |
|:---|:---|:---|
| **① 추론의 재귀 구조** | 자기회귀 모델은 지금까지 만든 토큰을 다시 입력에 넣어 다음 토큰을 예측한다. 생성은 이 되먹임의 반복이다 | 자기회귀적 생성 |
| **② 정확한 해의 불가능성** | 확률이 가장 높은 **시퀀스 전체**를 찾으려면 $\lvert T \rvert^L$ 개의 후보를 봐야 하므로 정확한 MAP 디코딩은 계산 불가능하다 | 탐색 공간의 크기 |
| **③ 근사 ①: 그리디와 빔** | 그리디는 매 시점 최선만 고르므로 전역적으로 차선에 빠질 수 있다. 빔 탐색은 상위 $B$ 개 부분 시퀀스를 함께 확장해 더 나은 해를 찾는다 | 탐색 트리 비교 |
| **④ 로그 확률을 쓰는 이유** | 확률을 곱해 나가면 길이가 늘수록 값이 0으로 언더플로된다. 로그를 취해 덧셈으로 바꾸면 수치적으로 안정된다 | log-softmax |
| **⑤ 근사 ②: 샘플링** | 항상 최선을 고르면 문장이 단조로워진다. Top-K는 상위 $K$ 개에서, Top-p는 누적 확률이 $p$ 를 넘을 때까지 모은 집합에서 뽑아 다양성과 품질을 절충한다 | Top-K vs Top-p |
| **⑥ 탐색의 비용** | 빔 탐색의 실행 시간은 빔 폭 $k$ 와 최대 길이에 대해 가파르게 증가한다. 품질을 위해 넓힌 빔은 그대로 비용이 된다 | 복잡도 측정 |
| **⑦ 낭비의 발견** | 토큰을 하나 늘릴 때마다 앞선 토큰들의 $K, V$ 를 처음부터 다시 계산한다. 이 값들은 이미 확정되어 변하지 않는데도 그렇다 | 중복 계산 |
| **⑧ KV 캐시** | 계산해 둔 $K, V$ 를 저장하고 새 토큰의 것만 이어 붙이면, 매 스텝의 쿼리는 **한 토큰뿐**이 된다. 인과 마스크와 위치 인덱스도 이에 맞춰 갱신해야 한다 | KVCache 통합 |
| **⑨ 실측** | 캐시 유무의 출력이 동일함을 확인한 뒤 속도를 재면, 시퀀스가 길어질수록 격차가 급격히 벌어진다 | 정합성 검증 · 속도 비교 |
