...
![Figure 1: 데이터의 구조와 요약 통계량 (Table 21.1). $A_0$에 따라 $L_1$의 분포가 달라지고, $L_1$에 따라 $A_1$과 $Y$의 평균이 달라지는 구조를 보여준다.](./images/table_21_1.png)

* 위 표(Table 21.1)를 트리 구조로 시각화하면 데이터의 생성 과정을 더 명확히 이해할 수 있습니다.

![Figure 2: 관찰된 데이터의 인과 트리 (Figure 21.1). 각 가지(Branch)는 처리($A$)와 교란요인($L$)의 경로를 나타내며, 각 끝단에는 관찰된 개체 수($N$)와 평균 결과($E[Y]$)가 표시되어 있다.](./images/figure_21_1.png)
...
![Figure 3: "항상 치료($a_0=1, a_1=1$)" 전략 하에서의 반사실적 트리 (Figure 21.2). $A_0$와 $A_1$의 확률은 1로 고정되지만, $L_1$의 조건부 확률($P(L_1|A_0)$)과 조건부 평균 결과($E[Y|\dots]$)는 원본 데이터에서 그대로 가져와 보존된다.](./images/figure_21_2.png)
...
![Figure 1: 시변 처리에서의 이중 강건 추정 흐름도. Step 1의 전향적 가중치 계산과 Step 2의 후향적 결과 회귀가 결합되는 구조를 보여줍니다.](./images/dr_time_varying_process.png)
