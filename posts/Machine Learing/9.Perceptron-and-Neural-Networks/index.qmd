...
* 여러 개의 은닉 유닛을 병렬 및 직렬로 쌓으면, 연산은 개별 for-loop가 아니라 벡터 및 행렬 연산(Vectorization)으로 표현됩니다.
* 층(layer) $r$에 대해 가중치 행렬 $W^{[r]}$를 도입하면 심층 네트워크의 계층적 연산은 다음과 같이 전개됩니다.
...
* 이 과정은 과거 머신러닝의 커널 기법(Kernel Method)과 긴밀히 연결됩니다. 딥러닝은 고정된 피처 맵을 사용하는 선형 모델과 달리, 최종 예측 직전 층까지의 연산 $a^{[r-1]} = \phi_\beta(x)$를 자동적으로 학습하여 문제에 적합한 최적의 피처 맵 $\phi_\beta(\cdot)$을 구축하는 과정으로 이해할 수 있습니다.
