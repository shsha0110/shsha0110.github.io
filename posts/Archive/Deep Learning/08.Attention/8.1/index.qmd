...
description: "LLaMA의 게이트 MLP와 어텐션을 밑바닥부터 구현하고 FLOPs를 직접 세어, 왜 컨텍스트 길이를 늘리는 일이 그토록 어려운지를 O(s²d) 항으로 설명합니다."
author: "유성현"
...
> 본 포스트는 서울대학교 컴퓨터공학부 **Hyun Oh Song** 교수님의 *Introduction to Deep Learning* (M2177.0043) 8강 실습 세션(*Modules in Transformers*)의 노트북을 바탕으로, 코드와 그 배경이 되는 논리를 함께 따라갈 수 있도록 재구성한 것입니다. 본문에서 사용하는 `Linear`, `Activation` 는 강의에서 미리 제공한 기본 빌딩블록(선형 사상과 활성화 함수)입니다.

## 한 줄 요약 (TL;DR)

> **"MLP는 토큰마다 따로 작용하고 어텐션은 토큰끼리 섞습니다. 이 차이가 곧 $\mathcal{O}(sd^2)$ 와 $\mathcal{O}(s^2 d)$ 의 차이이며, 컨텍스트 길이가 비싼 이유입니다."**
>
> 트랜스포머 블록의 두 핵심 모듈을 직접 만들어 보고, 각각의 연산량을 손으로 세어 실측과 대조합니다. **MLP**는 각 토큰에 독립적으로 작용하며, LLaMA 계열은 여기에 세 가지 설계를 얹습니다. 두 갈래 경로를 원소별 곱으로 합치는 게이트 구조(SwiGLU), 블록을 그대로 쌓을 수 있게 하는 입출력 차원 일치, 그리고 표현력을 위한 은닉 차원 확장입니다. **어텐션**은 반대로 토큰들 사이에 정보를 흐르게 하는 통로로, 쿼리·키의 유사도로 가중치를 만들어 값을 섞습니다. 두 모듈의 FLOPs를 세면 결정적인 차이가 드러납니다. 시퀀스 길이 $s$ 에 대해 MLP는 선형인 반면 어텐션에는 $\mathcal{O}(s^2 d)$ 항이 있어, 길이를 두 배로 늘리면 그 항만 네 배가 됩니다. "왜 LLM의 컨텍스트 길이를 늘리기가 그토록 어려운가"라는 질문의 답이 바로 이 항입니다. 마지막으로 자기회귀 생성을 위해 미래를 가리는 **인과적 마스킹**과, 표현력을 위해 어텐션을 여러 갈래로 나누는 **멀티헤드 어텐션**을 더합니다.
...
이 직렬(interleaved) 구조는 이후 등장할 **게이트 구조**와 대비되는 기준점(baseline)이 됩니다.
...
![Figure: LLaMA가 채택한 게이트 MLP의 구조도입니다. 아래쪽 입력 $x$ 에서 화살표가 **두 갈래로 갈라져** 각각 파란색 Linear 1과 Linear 2로 들어갑니다. 오른쪽 갈래인 Linear 1은 그대로 위로 올라가고, 왼쪽 갈래인 Linear 2는 분홍색 SiLU를 거친 뒤 올라가, 가운데 $\odot$ 기호에서 두 갈래가 **원소별 곱**으로 만납니다. 그 결과가 다시 Linear 3을 지나 출력이 됩니다. 앞 절의 직렬 MLP가 하나의 경로였던 것과 달리 여기서는 경로가 둘이며, SiLU를 통과한 쪽이 통과하지 않은 쪽의 각 원소를 얼마나 통과시킬지 조절하는 **게이트** 역할을 한다는 점이 이 구조의 핵심입니다. 위 수식 $\text{Linear}_3(\text{SiLU} \circ \text{Linear}_2(x) \cdot \text{Linear}_1(x))$ 를 그림으로 그대로 옮긴 것입니다.](./images/mlp_gated_llama.png)
...
![Figure: 스케일드 닷-프로덕트 어텐션(Scaled Dot-Product Attention)의 연산 흐름도입니다. 아래에서 $Q$ 와 $K$ 가 첫 번째 MatMul로 들어가 유사도 행렬을 만들고, 노란색 Scale에서 $\sqrt{d_k}$ 로 나뉜 뒤, 분홍색 Mask(opt.)를 선택적으로 거쳐, 초록색 SoftMax에서 행마다 합이 1인 어텐션 가중치가 됩니다. 이 가중치가 오른쪽에서 곧장 올라온 $V$ 와 두 번째 MatMul에서 만나 최종 출력이 됩니다. 이 그림이 말하는 바는 분명합니다. **각 토큰의 출력은 다른 모든 토큰의 값 $V$ 를 어텐션 가중치로 섞은 것**이며, 그 가중치는 쿼리와 키의 유사도가 결정합니다. 즉 어텐션은 토큰들 사이에 정보를 주고받게 하는 통로이고, Mask 단계는 뒤에서 다룰 인과적 마스킹이 끼어드는 자리입니다.](./images/attention_token_communication.png)
...
![Figure: 일반 Self-Attention(왼쪽)과 Causal-Attention(오른쪽)을 "The cat sat on the mat" 이라는 같은 문장 위에서 비교한 그림입니다. 아래쪽 주황색 점이 각 토큰, 위쪽 검은 막대가 그 위치의 출력이며, 회색 선은 어떤 토큰이 어떤 토큰을 참조하는지를 나타냅니다. 왼쪽에서는 모든 출력이 모든 입력과 빠짐없이 연결되어 있어 첫 번째 토큰조차 뒤에 오는 "mat" 을 볼 수 있습니다. 오른쪽에서는 연결선이 **왼쪽 아래 삼각형 방향으로만** 남아, 각 위치가 자기 자신과 그 이전 토큰만 참조합니다. 자기회귀 생성에서는 아직 만들어지지 않은 토큰을 미리 볼 수 없어야 하므로, 이 삼각 구조를 강제하는 것이 인과적 마스킹의 목적입니다.](./images/causal_masking.png)
...
| 단계 | 내용 | 근거 |
|:---|:---|:---|
| **① 두 모듈의 역할 분담** | MLP는 각 토큰에 **독립적으로** 작용하고, 어텐션은 토큰들 **사이에** 정보를 흐르게 한다. 트랜스포머 블록은 이 둘의 교대다 | 모듈별 동작 범위 |
| **② 게이트 MLP** | LLaMA는 $\text{Linear}_3(\text{SiLU} \circ \text{Linear}_2(x) \cdot \text{Linear}_1(x))$ 형태로, 한쪽 경로가 다른 쪽을 원소별로 통과시키는 게이트 역할을 한다 | SwiGLU 구조도 |
| **③ 나머지 두 설계** | 입출력 차원을 같게 맞추면 같은 블록을 그대로 쌓을 수 있고, 은닉 차원을 확장하면 표현력이 커진다 | 차원 일치 · 확장 |
| **④ MLP의 복잡도** | 토큰마다 독립이므로 FLOPs는 시퀀스 길이 $s$ 에 **선형**이고 차원 $d$ 에 대해서는 이차다: $\mathcal{O}(s d^2)$ | FLOPs 계산 · 런타임 실측 |
| **⑤ 어텐션의 절차** | $Q, K, V$ 투영 → $QK^\top$ 유사도 → $\sqrt{d_k}$ 스케일링 → softmax → $V$ 가중합. 각 출력은 모든 토큰의 값을 섞은 결과다 | 스케일드 닷-프로덕트 어텐션 |
| **⑥ 어텐션의 복잡도** | 투영은 $\mathcal{O}(s d^2)$ 이지만 유사도 행렬이 $s \times s$ 라서 $\mathcal{O}(s^2 d)$ 항이 추가된다. $s$ 를 두 배로 늘리면 이 항은 네 배가 된다 | FLOPs 계산 · 런타임 실측 |
| **⑦ 어텐션이 배우는 것** | 어텐션 가중치 행렬을 그려 보면, 최근 토큰에 가중치가 쏠리는 **위치 기반** 패턴과 의미가 비슷한 토큰끼리 묶이는 **의미 기반** 패턴이 모두 나타난다 | 어텐션 시각화 |
| **⑧ 인과적 마스킹** | 자기회귀 생성에서는 미래 토큰을 볼 수 없어야 하므로, 상삼각 영역을 $-\infty$ 로 채워 softmax 이후 가중치가 정확히 0이 되게 한다 | 마스크 행렬 · 균등 어텐션 확인 |
| **⑨ 멀티헤드** | 차원을 $H$ 갈래로 나눠 각각 독립적으로 어텐션을 수행하고 이어 붙이면, 전체 계산량은 거의 그대로 두면서 서로 다른 종류의 관계를 동시에 포착할 수 있다 | MHA 구조 · reshape vs transpose |
