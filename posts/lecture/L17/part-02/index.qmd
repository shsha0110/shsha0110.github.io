...
&= \sum_{w} P(y|do(x), do(w)) P(w|do(x)) \quad \text{(Composition Rule)} \\
&= \sum_{w} \underbrace{P(y|do(w))}_{Q[Y]} \underbrace{P(w|do(x))}_{Q[W]} \quad \text{(Rule 3: Deletion of } do(x) \text{)}
\end{aligned}
$$
...
\begin{aligned}
Q[Y] &= P(y|do(w)) \\
&= P(y|do(w, z)) \quad \text{(Rule 3: Exclusion of } Z \text{)} \\
&= \sum_{x} P(y|do(w, z), x) P(x|do(w, z)) \quad \text{(Conditioning on } X \text{)} \\
&= \sum_{x} P(y|do(w, z), x) P(x|do(z)) \quad \text{(Rule 3: } W \text{ does not affect } X \text{)} \\
&= \sum_{x} \underbrace{P(y|do(z), w, x)}_{\text{Available in Data}} \underbrace{P(x|do(z))}_{\text{Available in Data}} \quad \text{(Rule 2: Exchangeability)}
\end{aligned}
$$
...
\begin{aligned}
Q[W] &= P(w|do(x)) \\
&= P(w|do(x, z)) \quad \text{(Rule 3: Exclusion of } Z \text{)} \\
&= \underbrace{P(w|do(z), x)}_{\text{Available in Data}} \quad \text{(Rule 2: Exchangeability)}
\end{aligned}
$$
...
$$
* (여기서 $x'$는 내부 합의 더미 변수이며, 바깥쪽의 $x$와 구분하기 위해 사용되었습니다.)*
