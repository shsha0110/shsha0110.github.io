...
![Figure: XOR 문제의 선형 분리 불가능성. 왼쪽 그림에서 초록 십자(+)와 빨간 빼기(−) 기호로 표시된 네 점은 단 하나의 직선으로 분리할 수 없습니다. 오른쪽 그림의 물음표(?)처럼, 어떤 직선을 그어도 두 클래스를 완벽히 분리하는 선형 결정 경계를 찾을 수 없어 선형 함수로는 XOR을 표현하지 못함을 보여줍니다.](./images/figure_xor_problem.png)
...
![Figure: 신경망 유형 비교. (a) 순환 신경망(RNN)은 뉴런이 자기 자신으로 연결되는 루프가 있어 시계열 처리에 적합하고, (b) 순방향 신경망(FNN)은 방향성 비순환 그래프 구조로 정보가 한 방향으로만 흐릅니다. 오른쪽의 그래프 신경망(GNN)은 이웃 노드의 정보를 집계하는 Aggregator를 사용합니다.](./images/figure_fnn_types.png)
...
![Figure: 퍼셉트론 모델(Minsky-Papert, 1969). 세 입력 $x_1, x_2, x_3$가 각자의 가중치 $w_1, w_2, w_3$를 거쳐 뉴런(파란 원)에 전달되고, 활성화 함수를 통과한 뒤 출력 $y$를 생성합니다. 가중치가 입력의 중요도를 조절하고, 활성화 함수가 비선형성을 부여합니다.](./images/figure_perceptron.png)
...
![Figure: 시그모이드 함수의 그래프. 입력 $x$가 $-10$에서 $10$ 사이일 때 출력이 0에서 1로 S자 형태로 매끄럽게 증가합니다. $x$가 $\pm 4$를 넘어서면 기울기가 거의 0에 수렴하는 포화 현상이 뚜렷하게 나타나며, 이것이 깊은 네트워크에서 기울기 소실을 일으키는 원인이 됩니다.](./images/figure_sigmoid.png)
...
![Figure: ReLU 함수의 그래프. $x < 0$인 구간에서는 출력이 0으로 고정되고, $x \geq 0$인 구간에서는 $y = x$인 선형 함수처럼 동작합니다. 음수 구간에서의 기울기가 0이 되는 것이 dying ReLU 문제의 원인이지만, 양수 구간에서 기울기가 1로 유지되어 깊은 네트워크에서 기울기 소실 없이 효과적으로 학습됩니다.](./images/figure_relu.png)
...
![Figure: SGD의 직관적 이해. 손실 곡면($\mathcal{L}$ vs. $\theta$)에서 현재 위치에서의 기울기(gradient) 반대 방향으로 $\eta$만큼 이동하는 과정을 반복합니다. 경사가 양수이면 $\theta$를 줄이고, 음수이면 $\theta$를 늘려 손실 최솟값으로 내려갑니다.](./images/figure_sgd.png)
...
![Figure: 2-레이어 신경망의 계산 그래프. 왼쪽에서 오른쪽으로 $x \to h_1 \to h_2 \to \hat{y} \to \mathcal{L}$ 순서로 순방향(forward) 계산이 이루어지고, 역전파 시에는 $\mathcal{L}$로부터 각 노드로 그래디언트가 역방향으로 전달됩니다. $W_1, b_1, W_2, b_2$는 학습 대상인 파라미터(단말 노드)입니다.](./images/figure_computation_graph.png)
...
![Figure: 연쇄 법칙의 계산 그래프 표현. 위쪽 그래프는 $x \to u \to z$의 단순한 체인 구조이고, 아래쪽 그래프는 $x$로부터 $u$와 $v$ 두 경로가 분기되어 $z$에 합쳐지는 구조입니다. 아래 경우에는 두 경로의 기여를 합산하는 편미분의 합산 법칙이 적용됩니다.](./images/figure_chain_rule.png)
...
![Figure: 역전파의 흐름. 순방향 패스에서 $x \to h_1 \to h_2 \to \hat{y} \to \mathcal{L}$으로 값이 계산되고, 역방향 패스에서는 $\delta(\hat{y}) \to \delta(h_2) \to \delta(h_1)$ 순서로 그래디언트가 전파됩니다. 각 파라미터($W_1, b_1, W_2, b_2$)에 대한 그래디언트는 해당 노드에서 국소적으로 계산됩니다.](./images/figure_backprop_example.png)
