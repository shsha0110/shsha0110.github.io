...
> 본 포스트는 서울대학교 데이터 마이닝(Introduction to Data Mining) 강의(Spring 2026, 유재민 교수님)의 Time Series Forecasting 1 강의 자료를 바탕으로 작성되었습니다.
...
![Figure: 시계열 데이터의 다양한 예시. 왼쪽은 미국 자동차 소매 판매량(1976~2021년)의 단변량 시계열로 장기 트렌드와 주기성을 보여주고, 오른쪽은 풍력 발전기의 다섯 가지 센서 데이터(베어링 온도, 파워, 풍속, 고장 상태 등)를 동시에 보여주는 다변량 시계열이다. 두 그림 모두 시간에 따른 변동과 패턴이 내재되어 있음을 잘 보여준다.](./images/tsf1_timeseries_examples.png)
...
![Figure: 시계열 분류의 직관적 예시. 정상 동방 리듬(Normal Sinus Rhythm)의 ECG는 QRS 복합파가 일정하고 간격이 규칙적이며 분당 60~100회의 심박수를 보인다. 이 전체 시계열을 입력으로 받아 "건강한가(Healthy or not)?"라는 시계열 수준 레이블을 출력하는 것이 분류 문제의 전형적인 형태이다.](./images/tsf1_ecg_classification.png)
...
![Figure: 타임스탬프 수준의 시계열 예측 예시. SARIMA, ARIMA, Facebook Prophet, LSTM 등 여러 모델이 농산물 수확량(Yield) 예측에 활용된 결과를 비교한 그래프로, 각 모델이 시점별로 서로 다른 예측값(파란 곡선과 그 신뢰구간)을 산출함을 보여준다. 붉은 실선은 실제값이며, 미래 구간에서의 불확실성을 파란 음영으로 표현하고 있다.](./images/tsf1_forecasting_models_comparison.png)
...
![Figure: 시계열 예측의 직관적 개요. 파란 선은 과거의 관측된 시계열이며 등락을 반복하고 있고, 수직선을 기준으로 오른쪽의 붉은 선은 미래 구간에 대한 예측(forecast)이다. 핵심은 과거 패턴을 바탕으로 미래의 행동을 추론하는 것이다.](./images/tsf1_forecasting_intuition.png)
...
![Figure: 예측 문제의 설정을 도식화한 그림. 위쪽 시계열($z_t$)은 스파이크를 포함한 관측 시계열이고, 수직 초록선 오른쪽 구간이 예측 대상이다. 점선으로 표시된 여러 샘플 경로(predictions sample paths)는 미래에 대한 확률적 예측의 다양성을 나타낸다. 붉은 가로선은 예측 지평 $h$를 표시한다. 아래쪽 행렬($x_t$)은 각 시점에 추가적으로 관측되는 공변량(covariate)을 나타낸다.](./images/tsf1_forecasting_setup.png)
...
![Figure: 추가 속성(covariate)이 포함된 예측 설정. 위쪽 $z_t$는 주 예측 대상인 시계열이며, 아래쪽 붉은 배경으로 강조된 $x_t$ 행들은 예측 대상 구간(수직선 오른쪽)에서도 알려진 공변량이 있음을 나타낸다. 이러한 공변량 정보를 모델에 반영하면 예측 정확도를 높일 수 있다.](./images/tsf1_additional_attributes.png)
...
![Figure: 뉴스벤더 모델의 최적 해. 수요 $d$에 대한 누적분포함수(CDF) 곡선이 0에서 1로 단조증가하며, 최적 구매 수량 $q^*$는 이 CDF가 $(s-c)/c$와 만나는 지점의 $x$값에 해당한다. 이 그림은 단순 점 예측으로는 포착할 수 없는, 분포 전체를 활용한 의사결정의 필요성을 직관적으로 보여준다.](./images/tsf1_newsvendor_cdf.png)
...
![Figure: Naïve 방법 예시. 파란 선은 학습 시계열(약 40 스텝), 수직 점선은 예측 시작점이며, 초록 수평선이 Naïve 방법의 예측값이다. 마지막 관측값(약 25,000 수준)을 모든 미래 시점에 동일하게 사용함을 볼 수 있다.](./images/tsf1_naive_method.png)
...
![Figure: 평균 방법 예시. 파란 선은 학습 시계열이고, 초록 수평선이 Mean 방법의 예측값이다. Naïve 방법에 비해 마지막 관측값의 급락에 덜 반응하며, 더 안정적인 예측값(약 27,000 수준)을 출력함을 볼 수 있다.](./images/tsf1_mean_method.png)
...
![Figure: Naïve 계절성 방법 예시. 파란 선은 학습 시계열이며 주기적인 패턴이 뚜렷하게 반복된다. 예측 구간(점선 오른쪽)에서 초록 선이 바로 이전 시즌의 패턴을 그대로 복사하여, 실제 시계열의 계절적 피크와 골을 상당히 잘 따라가는 것을 볼 수 있다.](./images/tsf1_naive_seasonal.png)
...
![Figure: 시계열 분해의 예시. 위쪽 그래프는 원본 시계열로 점프, 완만한 상승 트렌드, 주기적 패턴이 혼재되어 있다. 아래 분해 결과에서 파란 선은 장기 트렌드, 붉은 선은 매끄러운 계절 성분, 초록 수직선은 갑작스러운 변화 이벤트(level shift)를 나타낸다. 이러한 분해를 통해 각 성분을 독립적으로 모델링할 수 있다.](./images/tsf1_decomposition.png)
...
![Figure: 오차 수정 함수의 도식. 수식 $\hat{z}_{t+1} = \hat{z}_t + \alpha(z_t - \hat{z}_t)$에서 첫 번째 항(이전 예측)과 두 번째 항(이전 예측의 오차 $\times$ 학습률)이 중괄호로 구분되어 표시되어 있다. 이 구조는 현재 예측이 이전 예측을 기반으로 오차를 보정하며 업데이트됨을 직관적으로 보여준다.](./images/tsf1_error_correction.png)
...
![Figure: 서로 다른 α 값에 따른 ETS 예측 비교. 파란 선은 학습 시계열, 초록 선($\alpha=0.9$)과 붉은 선($\alpha=0.1$)이 예측 구간에서 서로 다른 수준으로 시작됨을 볼 수 있다. $\alpha=0.9$는 직전 관측값에 강하게 반응하여 낮은 수준에서 시작하고, $\alpha=0.1$은 전체 과거 평균에 더 가까운 수준(약 27,000)에서 예측을 시작한다.](./images/tsf1_ets_comparison.png)
...
![Figure: ETS의 상태-예측 분리 다이어그램. 위쪽 행은 상태($l_0 \to l_1 \to l_2 \to \cdots \to l_{t-1}$)의 시간적 전이를 나타내고, 아래쪽 행은 각 상태에서 생성되는 예측값($\hat{z}_1, \hat{z}_2, \hat{z}_3, \hat{z}_t$)을 나타낸다. 상태는 이전 상태와 현재 오차를 받아 갱신되며, 예측값은 직전 상태에서 직접 출력된다.](./images/tsf1_state_prediction.png)
