...
인과추론(Causal Inference)의 핵심 목표 중 하나는 단순히 원인($X$)이 결과($Y$)에 미치는 **총 효과(Total Effect)**를 추정하는 것을 넘어, 그 효과가 발생하는 **메커니즘(Mechanism)**을 규명하는 것입니다. 이를 위해 우리는 효과를 **직접 효과(Direct Effect)**와 매개변수 $Z$를 경유하는 **간접 효과(Indirect Effect)**로 분해하게 됩니다.

[cite_start]이번 포스트에서는 강의 자료의 "Natural Indirect Effects: Formulation" 및 "Identification" 파트(Slide 2-6)를 중심으로, **Natural Indirect Effect(NIE)**의 정의와 **Total Effect(TE)**와의 관계, 그리고 관측 데이터로부터 이를 식별(Identification)하는 과정을 정리합니다[cite: 1, 2, 4].

![Figure: 매개 분석(Mediation Analysis)을 위한 인과 그래프(DAG). $X$가 $Y$에 미치는 직접 경로와 $Z$를 거쳐가는 간접 경로를 보여준다.](./images/mediation_dag_basic.png)
...
[cite_start]이 수식은 얼핏 복잡해 보이지만, 의미를 뜯어보면 직관적입니다[cite: 7, 8].
...
[cite_start]개별 단위(unit $u$)에 대한 효과를 모집단 전체로 확장하면 **평균 자연 간접 효과**는 다음과 같이 기댓값의 차이로 정의됩니다[cite: 9, 10].
...
인과 효과 분해(Decomposition)의 가장 큰 매력은 총 효과(TE)를 자연 직접 효과(NDE)와 자연 간접 효과(NIE)의 합으로 표현할 수 있다는 점입니다. [cite_start]강의 자료는 이 관계를 대수적으로 유도합니다[cite: 13].

### 3.1. Definitions of Effects

[cite_start]세 가지 주요 효과를 수식으로 정의하면 다음과 같습니다[cite: 14].
...
강의 자료는 흥미로운 대수적 관계를 통해 TE, NDE, NIE를 연결합니다. [cite_start]먼저, "역방향" NIE를 고려해 봅시다[cite: 19].

$$
-NIE(x^{*},x;Y) = -[\mathbb{E}(Y_{x,Z_{x^{*}}}) - \mathbb{E}(Y_{x})] = \mathbb{E}(Y_{x}) - \mathbb{E}(Y_{x,Z_{x^{*}}})
$$

[cite_start]이제 TE를 NDE와 위 식을 사용하여 다시 써보면 다음과 같습니다[cite: 20].
...
[cite_start]따라서, 다음과 같은 관계가 성립합니다[cite: 26].

$$
TE(x,x^{*};Y) = NDE(x,x^{*};Y) - NIE(x^{*},x;Y)
$$

[cite_start]이 관계는 다소 생소해 보일 수 있지만($-NIE(x^*, x)$ 항 때문), 결과적으로 우리가 흔히 사용하는 **Standard Additive Relation**을 도출해냅니다[cite: 27, 28].
...
![Figure: Total Effect의 분해 과정 시각화. 전체 효과($Y_x - Y_{x^*}$)는 직접 경로를 통한 효과($Y_{x,Z_{x^*}} - Y_{x^*}$)와 간접 경로를 통한 효과($Y_{x,Z_x} - Y_{x,Z_{x^*}}$)의 합으로 표현됨을 보여준다.](./images/te_decomposition_diagram.png)
...
[cite_start]강의 자료에서는 식별을 위해 다음과 같은 조건부 독립성을 가정합니다[cite: 31, 32].
...
[cite_start]위 가정이 성립하면, NIE는 관측 가능한 확률분포들의 조합으로 식별 가능합니다[cite: 34].

$$
NIE(x,x^{*};Y) = \sum_{w}\sum_{z} \mathbb{E}(Y \mid x^{*},z, w) [P(z \mid x, w) - P(z \mid x^{*}, w)] P(w)
$$

[cite_start]이 식은 다음과 같은 요소들로 구성됩니다[cite: 35]:
...
[cite_start]만약 모델이 **Markovian**이라면 (즉, 숨겨진 교란 요인이 없고 변수 간 관계가 DAG로 완벽히 표현된다면), 식은 훨씬 간단해집니다[cite: 36, 37].
...
이번 포스트에서는 강의 자료의 내용을 바탕으로 **Natural Indirect Effect**의 정의부터 식별 전략까지를 살펴보았습니다.

* **Definition:** NIE는 직접 경로를 차단($X=x^*$)하고 간접 경로만 활성화($Z_{x}$)했을 때의 효과입니다.
* **Decomposition:** $TE = NDE + NIE$의 관계를 가지며, 이는 효과가 경로별로 합산됨을 의미합니다.
* **Identification:** $Y_{x^*z} \perp Z_x | W$ 가정 하에, Mediation Formula를 통해 관측 데이터로 NIE를 추정할 수 있습니다.

### Content Checklist
아래는 강의 자료의 핵심 내용이 본 포스트에 포함되었는지 확인하는 체크리스트입니다.

| Category | Key Concept | Covered | Source Slide |
| :--- | :--- | :---: | :---: |
| **Definitions** | NIE Formulation ($Y_{x^*, Z_x} - Y_{x^*}$) | ✅ | Slide 2 |
| | Average NIE Definition | ✅ | Slide 2 |
| **Relations** | TE, NDE, NIE Definitions | ✅ | Slide 3 |
| | Decomposition Algebra ($TE = NDE - NIE_{rev}$) | ✅ | Slide 4-5 |
| | Additive Relation ($TE = NDE + NIE$) | ✅ | Slide 5 |
| **Identification** | Independence Assumption ($Y_{x^*z} \perp Z_x | W$) | ✅ | Slide 6 |
| | General Identification Formula (Sum over $w, z$) | ✅ | Slide 6 |
| | Markovian Simplification | ✅ | Slide 6 |

---
**References:**
본 포스트는 제공된 강의 자료 [Appendix. Direct and Indirect Effects.pdf]를 바탕으로 작성되었습니다.