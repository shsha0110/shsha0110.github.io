...
![Figure: 링크 팜의 전형적인 구조. 중앙의 타깃 페이지 t(초록 원)는 왼쪽 Accessible 페이지들(파란 원)로부터 직접 링크를 받고, 오른쪽 Owned 팜 페이지 1 ~ M(빨간 원)이 모두 t를 가리키며 PageRank를 집중시킨다. 스패머는 Owned 페이지 수 M을 자유롭게 늘려 t의 점수를 인위적으로 부풀릴 수 있다.](./images/link_farm.png)
...
![Figure: TrustRank의 링크 홉에 따른 신뢰도 감쇠. 신뢰 집합 출발점인 snu.ac.kr(스팸 비율 0%)에서 링크를 따라 이동할수록 스팸 비율이 0% → 0.1%(icml.cc) → 1%(my.blogger.com) → 10%(abc123.biz)로 급격히 증가한다. 원형 파이차트의 잘린 조각이 해당 도메인의 스팸 비율을 나타내며, 신뢰 집합으로부터 멀어질수록 신뢰도가 점진적으로 희석됨을 직관적으로 보여 준다.](./images/trustrank_hop.png)
...
![Figure: 근접도 예시 그래프. 쿼리 노드 A와 B(빨간 육각형)는 중간 노드 D, H 등을 경유해 연결되어 있으며, D에는 degree-1 리프 노드 E, F, G가 추가로 달려 있다. 이 구조에서 A-B 근접도를 어떻게 측정해야 가장 의미 있는지가 이어지는 절에서 논의될 핵심 질문이다.](./images/proximity_graph.png)
...
![Figure: 최단 경로 기반 근접도의 두 가지 한계. (왼쪽 위→아래) 노드 D에 degree-1 노드 E, F, G가 추가되어도 A와 B의 최단 거리는 2로 불변이다. (오른쪽 위→아래) A-D-B 경로 외 A-E-F-B 등의 추가 경로가 생겨도 최단 거리가 이를 반영하지 못하며, 여전히 A-B의 거리를 동일하게 평가한다.](./images/proximity_limitations.png)
...
![Figure: Random Walk with Restarts(PPR) 시뮬레이션. 동일한 근접도 예시 그래프에서 쿼리 노드 A(빨간 육각형)를 출발점으로 설정하고 반복 랜덤 워크를 실행한 결과, 붉은 굵은 화살표의 두께가 각 간선을 통과한 방문 빈도를 나타낸다. A와 직접 연결된 경로는 두껍고, D의 리프 노드(E, F, G) 방향으로는 얇아지는 것을 볼 수 있다. 이 방문 빈도가 각 노드와 A의 PPR 근접도 점수가 된다.](./images/rwr_flow.png)
...
![Figure: 현실 네트워크에서의 커뮤니티 구조. 노드들이 색상별(파랑, 빨강, 초록, 주황, 보라, 분홍)로 자연스러운 클러스터를 형성하고 있으며, 같은 색의 노드들은 내부 연결이 조밀하고 서로 다른 색 사이는 연결이 드문 것을 볼 수 있다. 커뮤니티 탐지는 이처럼 색 레이블 없이 구조만으로 이 클러스터를 자동으로 복원하는 과제이다.](./images/community_overview.png)
...
![Figure: 좋은 클러스터의 직관. 파란 점선이 클러스터 A(노드 1, 2, 3)와 나머지 V\A(노드 4, 5, 6)를 구분한다. 클러스터 내부에는 회색 간선이 촘촘하게 연결되어 있고, 점선을 가로지르는 굵은 검정 간선(클러스터 간 연결)은 단 2개뿐이다. 좋은 클러스터란 내부는 조밀하고 외부 연결은 최소인 부분집합이다.](./images/cut_concept.png)
...
![Figure: Cut Score 계산 예시. 클러스터 A = {1, 2, 3}이 베이지색 타원으로 표시되어 있고, 노드 1→5와 노드 3→4로 향하는 굵은 검정 간선 2개가 클러스터 경계를 넘는다. 오른쪽 화살표가 cut(A) = 2임을 명시한다.](./images/cut_score_calc.png)
...
![Figure: Cut Score의 한계 — Minimum Cut의 문제. 왼쪽(빨간 노드)과 오른쪽(초록 노드)의 두 밀집 커뮤니티를 분리하는 파란 점선("Optimal cut")이 직관적으로 올바른 분할이지만, 간선 단 1개만 끊으면 되는 오른쪽 외곽 고립 노드를 감싸는 빨간 점선("Minimum cut")이 cut score 최솟값으로 선택된다. 이는 내부 연결 밀도를 전혀 반영하지 못하는 cut score의 구조적 한계를 보여 준다.](./images/minimum_cut.png)
...
![Figure: Conductance 예시 비교. (왼쪽) 빨간 곡선 컷이 전체 파란 클러스터 상단의 외곽 노드 2개만 분리하며 φ = 2/4 = 0.5로 높다. (오른쪽) 빨간 직선 컷은 파란 노드 집합과 빨간 노드 집합 전체를 분리하며 φ = 6/92 ≈ 0.065로 훨씬 낮다. 직관적으로 올바른 오른쪽 분할이 Conductance도 낮아, Cut Score 단독보다 Conductance가 우수한 척도임을 보여 준다.](./images/conductance_comparison.png)
...
![Figure: 로컬 커뮤니티 탐지 예시. 다양한 색의 클러스터가 공존하는 네트워크에서 보라색 클러스터 안의 한 노드가 시드 노드(빨간 화살표로 표시)로 주어진다. 알고리즘의 목표는 전체 그래프를 탐색하지 않고도 시드 노드 주변의 보라색 클러스터만 선형 시간에 복원하는 것이다.](./images/local_community_seed.png)
...
![Figure: Sweep Curve 예시. x축은 PPR 점수 내림차순으로 정렬된 노드 랭크 i, y축은 누적 집합 A_i의 Conductance φ(A_i)이다. 랭크 약 10과 18 부근에서 두 개의 지역 최솟값(보라색 화살표로 "Good clusters"라 표시)이 나타나며, 이 두 지점이 좋은 클러스터 경계 후보가 된다. 랭크 1에서는 단일 노드이므로 φ = 1에서 시작하여 감소한다.](./images/sweep_curve.png)
...
![Figure: APPR의 근사 오차. x축은 노드 인덱스(시드로부터의 거리 순), y축은 Cutsize이다. 두 곡선이 약 8,000번 노드 이전까지는 거의 겹치다가 이후부터 점선(Approximate PPR)이 실선(Full PPR)보다 급격하게 상승하며 벌어진다. 이는 ε 임계값 때문에 시드에서 먼 노드들의 잔차가 조기에 종료되어 과소평가되기 때문이다.](./images/appr_error_chart.png)
