...
![Figure: PageRank 점수로 순위가 매겨진 웹 그래프. 오른쪽 그림에서 노드 B(38.4)와 C(34.3)는 서로 강하게 연결되어 있어 가장 높은 점수를 받는다. 왼쪽은 실제 웹 그래프에서 두 허브 노드(파란 점, 빨간 점) 사이에 수많은 엣지가 집중되는 구조를 보여준다.](./images/pagerank_network_overview.png)
...
![Figure: 순환 정식화 예시. 노드 j는 아웃-차수 3이므로 각 아웃링크의 가중치가 r_j/3이다. 노드 k는 아웃-차수 4이므로 r_k/4이다. 노드 i의 PageRank r_i는 이 두 기여의 합으로 계산된다.](./images/pagerank_recursive_equation.png)
...
![Figure: y-a-m 세 노드 그래프와 행렬 정식화. y는 자신과 a에 링크하고, a는 y와 m에 링크하고, m은 a에만 링크한다. 전이 행렬 M의 (i,j) 원소는 j에서 i로 이동할 확률인 1/d_j (또는 0)이다.](./images/pagerank_matrix_example.png)
...
![Figure: Dead Ends 문제 예시. m 노드는 아웃링크가 없어 전이 행렬의 m 열이 모두 0(빨간색)으로 표시된다. 열 확률 행렬 조건이 깨지므로, 반복 계산 시 PageRank 점수의 합이 단계마다 줄어들어 결국 모두 0으로 수렴한다.](./images/pagerank_dead_ends.png)
...
![Figure: Dead Ends 해결책 (Always Teleport). 왼쪽: m 열이 모두 0인 원래 행렬(Dead End). 오른쪽: m 열의 0을 모두 1/3(= 1/N)으로 교체한 수정된 행렬. 초록 점선 화살표는 m에서 전 페이지로의 텔레포트 엣지를 나타내며, 이로써 M'이 다시 열 확률 행렬이 된다.](./images/pagerank_dead_ends_solution.png)
...
![Figure: Spider Trap 문제 예시. m 노드에 m→m 자기 루프가 있어 Spider Trap을 형성한다. 전이 행렬의 m 열에서 M[m][m]=1임을 확인할 수 있다. 반복 계산 결과 PageRank 점수가 점점 m에 집중되어, 최종적으로 r_m=1, r_y=r_a=0으로 수렴한다.](./images/pagerank_spider_traps.png)
...
![Figure: Spider Trap 해결책 (Probabilistic Teleport). 왼쪽: m에 자기 루프가 있는 원래 그래프(Spider Trap). 오른쪽: 초록 점선 화살표(텔레포트 엣지)가 추가된 수정 그래프. 랜덤 서퍼는 확률 (1-β)로 m을 포함한 어느 노드에서든 전 그래프로 이동할 수 있어, 트랩에서 빠져나올 수 있게 된다.](./images/pagerank_spider_traps_solution.png)
...
![Figure: Google Matrix 수치 예시. β=0.8 적용 결과 A를 구성하고 Power Iteration을 수행한다. 그래프의 각 방향 엣지에 전이 확률(검정: 원래 링크 가중치, 초록: 텔레포트 가중치)이 표기되어 있으며, Spider Trap인 m의 점수(21/33≈0.636)가 가장 높지만 텔레포트 덕분에 y(7/33)와 a(5/33)에도 양의 PageRank가 배분된다.](./images/pagerank_google_matrix.png)
...
![Figure: Sparse Matrix Encoding 예시. 소스 노드 번호, 아웃-차수, 목적지 노드 목록만을 저장하여 비영 원소만 기록한다. 예컨대 소스 노드 0은 차수 3, 목적지 {1,5,7}; 소스 1은 차수 5, 목적지 {17,64,113,117,245}; 소스 2는 차수 2, 목적지 {13,23}. 이 표 형식이면 수십억 링크도 디스크에 저장 가능하다.](./images/sparse_matrix_encoding_table.png)
...
![Figure: 기본 알고리즘의 메모리-디스크 흐름. 왼쪽 r_new 벡터(메모리)와 오른쪽 r_old 벡터(메모리) 사이에 희소 행렬 테이블(디스크)이 위치한다. 소스 0(차수 3, 목적지 1·5·6) 행이 강조 표시되어, r_old[0]을 읽고 3개 목적지에 β·r_old[0]/3씩 r_new를 갱신하는 한 스텝의 동작을 보여준다.](./images/basic_algorithm_memory_flow.png)
