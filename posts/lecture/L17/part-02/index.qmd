---
title: "[Causal Inference] Experimental Identifiability & g-ID"
description: "Combining Heterogeneous Experiments: z-ID and General Identifiability"
author: "유성현"
date: "2026-01-25"
...
# 1. Overview: The Challenge of Experimental Conditions

이전 포스트에서는 Causal Data Science가 다루는 4가지 차원(Dimensions)에 대해 개괄적으로 살펴보았습니다. [cite_start]이번 포스트에서는 그 첫 번째이자 가장 기초가 되는 문제인 **Experimental Conditions**와 **General Identifiability (g-ID)**에 대해 깊이 있게 다룹니다[cite: 274].

우리가 $do(x)$에 대한 인과 효과 $P(y|do(x))$를 알고 싶을 때, 이상적인 상황은 $X$에 대한 무작위 대조군 실험(RCT) 데이터가 있는 것입니다. 하지만 현실에서는 다음과 같은 제약이 따릅니다:
...
가장 단순한 형태의 대리 실험 문제를 살펴보겠습니다. [cite_start]우리의 목표는 콜레스테롤 수치($X$)가 심장마비($Y$)에 미치는 인과 효과 $P(y|do(x))$를 알아내는 것입니다 [cite: 283-294].
...
[cite_start]만약 $Z$가 $X$를 통해서만 $Y$에 영향을 미친다면(즉, $Z \to Y$ 직접 경로가 없고, $Z$와 $Y$ 사이의 교란이 없다면), 우리는 다음 식을 유도할 수 있습니다[cite: 309].
...
모든 경우에 대리 실험이 유효한 것은 아닙니다. [cite_start]그래프 구조에 따라 식별 가능 여부가 달라집니다 [cite: 311-393].
...
[cite_start]조금 더 복잡한 그래프를 통해, 식별 불가능해 보이는 효과를 도출해 내는 과정을 수식으로 살펴보겠습니다 [cite: 394-405].
...
Do-calculus와 확률의 법칙(Law of Total Probability)을 사용하여 식을 분해해 봅시다. [cite_start]매개변수 $W$를 도입하여 conditioning을 수행합니다[cite: 395].
...
이제 우변의 두 항($P(y|do(w))$, $P(w|do(x))$)을 각각 구해야 합니다. [cite_start]놀랍게도 이 두 항은 모두 $P(v|do(z))$로부터 식별 가능합니다 [cite: 405-408].
...
이 이론의 가장 강력한 응용 사례는 **약물 상호작용(Drug-Drug Interaction)** 분석입니다. [cite_start]개별 약물 실험 데이터만 있을 때, 두 약물을 동시에 처방했을 때의 효과를 예측할 수 있을까요? [cite: 415-460]
...
### 4.2. Derivation
우리는 $P(y|do(x_1, x_2))$를 구해야 합니다. [cite_start]혈압 $B$가 $X_1$과 $Y$ 사이의 매개체 역할을 한다는 점에 착안하여 식을 전개합니다 [cite: 570-577].
...
[cite_start]위의 사례들을 일반화하면 **General Identifiability (g-ID)** 알고리즘을 만들 수 있습니다 [cite: 488-511].
...
### Significance
이 알고리즘과 Do-calculus는 Experimental Identifiability 문제에 대해 **Completeness(완전성)**를 가집니다. [cite_start]즉, 이 알고리즘으로 식별할 수 없다면, 그 인과 효과는 주어진 데이터와 그래프 가정하에서는 이론적으로 식별이 불가능한 것입니다[cite: 586].
