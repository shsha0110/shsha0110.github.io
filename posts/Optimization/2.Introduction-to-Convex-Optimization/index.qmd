...
* **Perspective Function (투시 함수):** 함수 $f: \mathbb{R}^n \to \mathbb{R}$가 볼록일 때, $g: \mathbb{R}^{n+1} \to \mathbb{R}$로 정의되는 다음 함수도 볼록입니다.
    $$g(x, t) = t f(x/t), \quad \text{dom}(g) = \{(x, t) \mid x/t \in \text{dom}(f), t > 0\}$$
    이 연산은 KL-divergence 등 다양한 정보 이론적 척도나 기하학적 문제를 볼록 최적화 형태로 변환할 때 유용하게 사용됩니다.
* **Composition (합성):**
...
* **Optimal Value ($OPT$):**
    ...
    $$
* **Solvability:** $f(x^*) = OPT$를 만족하는 $x^* \in C$가 존재하면 **Solvable**이라 합니다.
...
기대 수익은 최대화하고 리스크는 최소화하는 문제는 다음과 같습니다.
$$
\text{maximize} \quad \mu^\top x - \gamma x^\top \Sigma x \quad \text{subject to} \quad 1^\top x = 1, x \in C'
$$
이 문제는 **Concave Maximization** 문제(목적함수가 오목함수)입니다. 이를 표준적인 볼록 최적화 형태로 바꾸기 위해 부호를 반전시켜 **Convex Minimization** 문제로 변환합니다.
