...
지금까지 우리는 Conformal Prediction(CP)의 다양한 이론적 확장(Extension)들을 다루었습니다. 이제 이 도구들이 실제 머신러닝 문제에서 어떻게 작동하는지 확인할 차례입니다.
...
단순히 정답을 포함하는 것을 넘어, **"정답 객체 중 90% 이상을 찾아내라(Recall $\ge$ 90%)"**와 같은 요구사항이 있을 때 사용합니다. 이를 위해 **False Negative Rate (FNR)**를 제어합니다.
...
![Figure 13: MS COCO 데이터셋에 대한 Multilabel Classification 결과 ($\alpha=0.1$). 빨간색 텍스트는 놓친 정답(False Negative), 파란색은 잘못 예측한 오답(False Positive), 검은색은 맞춘 정답(True Positive)을 나타낸다. 평균적으로 90% 이상의 정답 라벨을 찾아내고 있다.](images/figure13_multilabel.png)
...
![Figure 14: 종양 분할(Tumor Segmentation) 결과 ($\alpha=0.1$). 붉은 영역은 모델이 놓친 종양 부위(False Negative)이다. CRC를 통해 놓치는 부위를 통계적으로 최소화할 수 있다.](images/figure14_segmentation.png)
...
![Figure 15: 시계열 기온 예측 결과. (왼쪽) 일반 CP(주황색)는 분포 변화 시 커버리지가 무너지지만, Weighted CP(파란색)는 빠르게 회복하여 목표 커버리지(0.9)를 유지한다. (오른쪽) 예측된 구간의 시각화.](images/figure15_weather.png)
...
![Figure 16: 유해 댓글 탐지 예시 ($\alpha=0.1$). 다국어 댓글에 대해 정상 댓글을 유해하다고 잘못 분류할 확률을 10%로 제한하면서, 실제 유해 댓글의 70%를 성공적으로 잡아냈다.](images/figure16_toxic.png)
...
![Figure 17: ImageNet에 대한 Selective Classification 결과. (왼쪽) $\lambda$가 커질수록(가로축), 예측을 수행하는 비율(주황색)은 줄어들지만, 정확도(파란색)는 올라간다. 점선은 목표 정확도 90%를 달성하는 지점을 나타낸다. (오른쪽) 모델이 예측한 예시(가오리)와 기권을 선택한 예시(여우).](images/figure17_selective.png)
