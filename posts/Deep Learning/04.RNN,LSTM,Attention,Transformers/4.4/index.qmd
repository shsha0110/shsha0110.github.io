...
### 6.1 Greedy Search
매 스텝마다 가장 확률이 높은 토큰을 선택하는 가장 직관적인 방법입니다. 

#### **[Algorithm: Greedy Search]**
* **Input**: 주어진 토큰 시퀀스 $[t_1, \dots, t_n]$, 최대 길이 제한 $L$
* **Initialize**: $ind = n$
* **Repeat**
  1. **Update index**: $ind \leftarrow ind + 1$
  2. **Greedy generation**: $t_{ind} = \text{argmax}_{t \in \mathbb{T}} p(t | t_1, \dots, t_{ind-1})$
* **Until** 문장 종료 토큰(EOS)이 생성되거나 $ind$가 $L$에 도달할 때까지 반복
* **Output**: $[t_1, \dots, t_{ind}]$

* *단점*: 당장 확률이 높은 것만 쫓아가다 보니, 문장 전체로 보았을 때는 최적이 아닌(Suboptimal) 텍스트가 생성될 위험이 큽니다.

![Figure: Greedy search 전략 시각화.](./images/greedy_search.png)

### 6.2 Beam Search
매 스텝마다 모든 가능한 토큰을 확장한 뒤, 전체 시퀀스의 누적 로그 확률(Log-probability)이 가장 높은 상위 $B$개의 시퀀스(Beam width)만을 유지하는 방법입니다.

#### **[Algorithm: Beam Search]**
* **Input**: 접두사 토큰(prefix tokens) $[t_1, \dots, t_n]$, Beam width $B$, 최대 길이 제한 $L$
* **Initialize**: $Beam = \{([t_1, \dots, t_n], 0)\}$ (형태: 시퀀스, 로그 확률)
* **Repeat**
  1. Beam 내의 각 시퀀스 $s$에 대해 모든 토큰 $t \in \mathbb{T}$의 조건부 확률 $p(t|s)$를 계산합니다.
  2. 모든 후보를 확장(Expand)합니다: $(s, l) \rightarrow (s+t, l + \log p(t|s))$
  3. 확장된 모든 시퀀스를 풀(pool)에 수집합니다.
  4. 누적 로그 확률이 가장 높은 상위 $B$개의 시퀀스만 유지합니다.
* **Stop** 모든 시퀀스가 EOS로 끝나거나 길이 $L$에 도달하면 종료합니다.
* **Output**: 가장 확률이 높은 시퀀스를 반환합니다.

![Figure: Beam search 전략 시각화.](./images/beam_search.png)

### 6.3 Top-K Sampling
다음 토큰의 확률 분포에서 가장 확률이 높은 상위 $K$개의 토큰 집합을 찾고, 그 안에서 확률에 비례하여 무작위로 샘플링합니다.

#### **[Algorithm: Top-K Sampling]**
* **Input**: 정수 $K$, 주어진 토큰 시퀀스 $[t_1, \dots, t_n]$, 최대 길이 제한 $L$
* **Initialize**: $ind = n$
* **Repeat**
  1. **Update index**: $ind \leftarrow ind + 1$
  2. **Find top-K tokens**: $t_{ind}$에 대해 가장 확률이 높은 상위 $K$개의 토큰 집합 $\mathbb{T}_{top-K}$를 찾습니다.
  3. **Sample the next token**: 예측된 확률에 비례하여 $\mathbb{T}_{top-K}$ 안에서 $t_{ind}$를 샘플링하여 생성합니다.
* **Until** 문장 종료 토큰(EOS)이 생성되거나 $ind$가 $L$에 도달할 때까지 반복
* **Output**: $[t_1, \dots, t_{ind}]$

### 6.4 Nucleus Sampling (Top-p)
미리 정해둔 임계값(Threshold) $\eta$를 기준으로, 누적 확률의 합이 $\eta$를 넘는 최소한의 상위 토큰 집합($\mathbb{T}_{top-N}$)을 찾아 샘플링합니다. 예측의 확실성에 따라 후보 토큰의 개수 $N$이 유동적으로 변하여 더욱 자연스러운 텍스트 생성을 돕습니다.

#### **[Algorithm: Nucleus Sampling]**
* **Input**: 임계값 $\eta$, 주어진 토큰 시퀀스 $[t_1, \dots, t_n]$, 최대 길이 제한 $L$
* **Initialize**: $ind = n$
* **Repeat**
  1. **Update index**: $ind \leftarrow ind + 1$
  2. **Find $N$**: 상위 $N$개 토큰의 확률 합이 $\eta$보다 커지는 가장 작은 자연수 $N \in \mathbb{N}$을 찾습니다.
     $$\sum p(t_{ind} \in \mathbb{T}_{top-N} | t_1, \dots, t_{ind-1}) > \eta$$
  3. **Sample the next token**: 예측된 확률에 비례하여 $\mathbb{T}_{top-N}$ 안에서 $t_{ind}$를 샘플링하여 생성합니다.
* **Until** 문장 종료 토큰(EOS)이 생성되거나 $ind$가 $L$에 도달할 때까지 반복
* **Output**: $[t_1, \dots, t_{ind}]$
...
![Figure: 보상 모델(Reward Model) 학습 과정. 프롬프트 데이터셋을 초기 언어 모델(Initial Language Model)에 통과시켜 여러 텍스트를 생성한 뒤, 사람이 직접 응답의 질을 평가하여 순위(Human Scoring)를 매깁니다. 이렇게 구축된 텍스트-보상 점수(sample, reward) 쌍을 바탕으로, 주어진 텍스트의 인간 선호도를 예측하는 보상 모델을 학습하는 흐름을 화살표로 보여줍니다.](./images/rlhf_training.jpg)
...
![Figure: 보상 모델을 활용한 강화학습 파인튜닝(RLHF) 과정. 프롬프트가 주어졌을 때 학습 중인 언어 모델(Tuned Language Model)이 생성한 텍스트를 보상 모델이 평가합니다. 동시에 파라미터가 고정된 초기 언어 모델(Initial Language Model)이 생성한 기준 텍스트(Base Text)와의 차이를 KL 발산(KL prediction shift penalty)으로 계산하여 패널티를 부여합니다. 이 두 값을 합산한 최종 보상을 바탕으로 PPO(Proximal Policy Optimization) 알고리즘을 통해 튜닝 중인 언어 모델의 파라미터를 업데이트하는 순환 구조를 상세히 도식화한 그림입니다.](./images/rlhf_fine_tuning.jpg)