...
![Figure 1: 다층 GNN의 계층적 구조. 하단의 GNN Layer 2를 통해 2-hop 이웃들의 정보가 중간 노드들(파란색, 초록색, 빨간색 점)로 모이고, 이 정보가 다시 상단의 GNN Layer 1을 거쳐 최종 타겟 노드(노란색 점)의 임베딩으로 수렴되는 상향식 트리 구조를 보여줍니다.](./images/gnn-multilayer-hierarchy.png)
...
![Figure 6: 3단계 자가 연결(Self-Connection) 및 업데이트. 왼쪽의 순환 화살표가 있는 노드 'a' 모식도는 노드가 자기 자신의 기존 정보도 잃지 않고 보존해야 함을 상징합니다. 오른쪽 도식 상단의 최종 출력 단계에서, 집계된 이웃 정보와 노드 본연의 이전 상태가 결합되어 업데이트가 이루어집니다.](./images/gnn-step3-self-connection.png)
...
* **GraphSAGE 설계의 3가지 핵심:**
  ...
  * 2. **독립적인 가중치 부여**: $\text{CONCAT}$ 연산 직후 가중치 행렬 $W^{(l)}$을 곱하면, 행렬의 앞부분은 내 정보 전용 가중치($B^{(l)}$)로, 뒷부분은 이웃 정보 전용 가중치($W^{(l)}$)로 작용하여 정보를 더 정교하게 분리해 학습할 수 있습니다.
