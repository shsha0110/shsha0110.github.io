...
## 2. Encoder and Prior (Unchanged)

놀랍게도, 이산형 데이터를 다룸에도 불구하고 **Encoder(Inference Model)와 Prior는 연속형 모델과 동일**하게 유지됩니다.
...
## 3. Decoder Modification (Categorical Likelihood)

변화가 필요한 부분은 잠재 변수 $Z$에서 다시 데이터 $X$를 복원하는 **Decoder(Generative Model)** 파트입니다. $X$가 이산형이므로, 더 이상 가우시안 분포를 가정할 수 없습니다.
...
수식으로 표현하면 다음과 같습니다 (식 10):

$$
P_X = \text{softmax} \left( \text{MLP} \big( (I - A^T)^{-1} Z, W^3, W^4 \big) \right) \quad \cdots (10)
$$

여기서 `softmax`는 각 행(Row-wise)에 대해 적용되어, 각 변수의 범주별 확률 합이 1이 되도록 만듭니다.

![Figure: 이산형 변수를 처리하기 위한 DAG-GNN 아키텍처의 변화. Encoder 부분은 동일하지만, Decoder의 출력이 평균/분산($M_X, S_X$)에서 확률 분포($P_X$)로 변경된다. 이는 마지막 레이어에 Softmax가 적용됨을 의미한다.](./images/dag_gnn_architecture_discrete.png)

---

## 4. Loss Function Modification (Cross-Entropy)
...
* **데이터 표현:**
    * [x] 각 변수가 cardinality $d$를 갖는 유한 지지 집합임 명시
    * [x] 입력 $X$가 One-hot vector로 표현됨 설명
* **모델 구조 (유지되는 부분):**
    * [x] Prior와 Encoder 구조는 연속형과 동일함을 설명
    * [x] $q(Z|X)$는 여전히 Factored Gaussian임 언급
* **모델 구조 (변경되는 부분):**
    * [x] Likelihood $p(X|Z)$가 Factored Categorical Distribution임 명시
    * [x] Decoder 출력 $P_X$ (Probability matrix) 정의
    * [x] 식 (10) $P_X = \text{softmax}(\text{MLP}((I-A^T)^{-1}Z))$ 정확히 기술
    * [x] $f_2$가 Identity에서 Row-wise Softmax로 변경됨 설명
* **손실 함수:**
    * [x] KL Divergence 항(식 8)은 그대로 유지됨 언급
    * [x] Reconstruction Loss 식 (11) (Cross-Entropy 형태) 정확히 유도 및 기술

---
title: "[Paper Review] DAG-GNN: Theoretical Connection to Linear SEM"
description: "DAG-GNN이 Linear SEM(NOTEARS)의 일반화된 형태임을 증명하는 수학적 유도 과정과 통찰"
author: "유성현"
date: "2026-01-31"
categories: [Causal Inference, Linear Algebra, Theory Verification]
format:
  html:
    toc: true
    number-sections: false
...
## 1. Step 1: From VAE to Plain Autoencoder
...
### 1.2. Correspondence of Loss Functions
...
## 2. Step 2: From Nonlinear to Linear (The Core Derivation)
...
위 두 식을 결합해 볼까요? Decoder 식의 $Z$ 자리에 Encoder 식을 대입합니다.
...
즉, 선형 모델 하에서는 입력 $X$가 손실 없이 완벽하게 복원($\hat{X} = X$)됩니다. 따라서 손실 함수의 첫 번째 항인 **Reconstruction Error는 0**이 되어 사라집니다.

---

## 3. Deriving the NOTEARS Loss

이제 남은 것은 두 번째 항인 **Regularization Term** 뿐입니다. 여기에 Linear Encoder 식 $Z = (I - A^T)X$를 대입하여 정리해 봅시다.
...
* **개념적 흐름:**
    * [x] "Reverse thought flow" (역방향 사고) 개념 설명
    * [x] VAE $\to$ Plain Autoencoder $\to$ Linear Model로의 축소 과정 서술
* **수식 유도 (Step 1):**
    * [x] VAE ELBO의 각 항(Reconstruction, KL)이 결정론적 손실(MSE, L2 Reg)과 대응됨을 설명
    * [x] $\hat{X}$ (Decoder output) 표기법 도입
* **수식 유도 (Step 2):**
    * [x] $f_1 \dots f_4$를 Identity로 가정
    * [x] Linear Encoder/Decoder 식 기술 및 Perfect Reconstruction ($\hat{X}=X$) 증명
* **결론:**
    * [x] 최종 식 (12) $\frac{1}{2} \|(I-A^T)X\|_F^2$ 유도
    * [x] 이것이 Zheng et al. (2018)의 Least-squares loss와 동일함을 명시

---
title: "[Paper Review] DAG-GNN: A Novel Acyclicity Constraint"
description: "Matrix Exponential을 넘어: 수치적 안정성과 구현 편의성을 위한 다항식(Polynomial) 제약조건의 제안"
author: "유성현"
date: "2026-01-31"
categories: [Causal Inference, Optimization, Mathematics]
format:
  html:
    toc: true
    number-sections: false
...
앞선 섹션들에서 우리는 VAE 기반의 손실 함수(ELBO)와 선형/비선형 모델링을 정의했습니다. 하지만 여기에는 치명적인 허점이 하나 있습니다. ELBO를 최대화하든, Least-squares loss를 최소화하든, 학습된 인접 행렬 $A$가 **DAG(비순환 그래프)**라는 보장이 없다는 점입니다.

그래프 $G$가 인과관계 모델이 되기 위해서는 반드시 사이클(Cycle)이 없어야 합니다. 이번 포스트에서는 이 조합적(Combinatorial) 제약 조건을 어떻게 연속적인(Continuous) 수식으로 변환하여 최적화 과정에 통합했는지 살펴봅니다.

---

## 1. Motivation: Trace and Cycles

그래프 이론에서 인접 행렬의 거듭제곱은 경로(Path)와 깊은 연관이 있습니다.

### 1.1. Path Counting Logic
...
## 2. The Matrix Exponential (Previous Work)
...
## 3. Proposed Solution: Polynomial Constraint

저자들은 위 문제를 해결하기 위해, 행렬 지수 대신 **다항식(Polynomial)** 형태의 새로운 제약 조건을 제안합니다.

### Theorem 1 (Polynomial Acyclicity)
$A \in \mathbb{R}^{m \times m}$를 유향 그래프의 가중치 인접 행렬이라고 합시다. 임의의 양수 $\alpha > 0$에 대하여, 다음 조건이 성립하면 그래프는 Acyclic입니다.
...
## 4. Stability Analysis (Why Polynomial?)
...
* **Motivation:**
    * [x] 최적화 손실 함수만으로는 Acyclicity가 보장되지 않음 언급
    * [x] $B^k$의 $(i, j)$ 요소가 길이 $k$의 경로를 의미함 설명
    * [x] $B^k$의 대각 성분이 사이클을 의미함 설명
* **Existing Method (NOTEARS):**
    * [x] Matrix Exponential 제약식 $\text{tr}(e^{A \circ A}) - m = 0$ 포함
    * [x] Taylor series expansion 설명
    * [x] 구현상의 난점(자동미분, 수치 불안정) 지적
* **Proposed Method (Polynomial):**
    * [x] Theorem 1 내용 (조건 식 13) 정확히 기술
    * [x] $m$차 거듭제곱을 사용하는 이유 (DAG 최장 경로 $m-1$) 논리적 설명
* **Analysis:**
    * [x] Theorem 2 부등식 $(1 + \alpha|\lambda|)^m \le e^{c|\lambda|}$ 포함
    * [x] Theorem 2가 수치적 안정성을 의미함을 해석
    * [x] $\alpha$ 설정 방법(Spectral radius, Perron-Frobenius) 언급

---
title: "[Paper Review] DAG-GNN: Training Strategy with Augmented Lagrangian"
description: "Constrained Optimization의 해결: 증강 라그랑주(Augmented Lagrangian) 기법을 통한 ELBO와 Acyclicity의 통합 학습"
author: "유성현"
date: "2026-01-31"
categories: [Optimization, Causal Inference, Machine Learning]
format:
  html:
    toc: true
    number-sections: false
...
이제 이 두 가지를 하나로 묶어 실제 학습을 수행하는 **최적화 전략(Optimization Strategy)**을 다룰 차례입니다. 이 문제는 전형적인 **비선형 등식 제약 최적화(Nonlinear Equality-Constrained Optimization)** 문제입니다.
...
이러한 제약 조건이 있는 최적화 문제를 풀기 위해, 저자들은 **증강 라그랑주(Augmented Lagrangian)** 방법을 사용합니다. 이는 표준적인 라그랑주 승수법에 페널티 항(Penalty term)을 추가하여 수치적 안정성과 수렴성을 높인 기법입니다 (Bertsekas, 1999).
...
## 3. Algorithm: Iterative Update Rule

학습은 **$c$를 점진적으로 증가**시키면서 일련의 비제약 최적화 문제(Unconstrained optimization)를 푸는 방식으로 진행됩니다. 구체적인 알고리즘은 다음의 반복(Iteration) 과정으로 요약됩니다.
...
이 단계(Subproblem)는 경사 하강법(Gradient Descent)과 같은 Blackbox Stochastic Optimization Solver(예: Adam)를 사용하여 해결합니다. ELBO가 샘플 기반으로 정의되므로 확률적(Stochastic) 최적화가 적합합니다.
...
* **문제 정의:**
    * [x] 목적 함수 $f(A, \theta) = -L_{\text{ELBO}}$ 명시
    * [x] 제약 조건 $h(A) = 0$ 명시
    * [x] 최적화 변수 $A$와 $\theta$ 정의
* **최적화 기법:**
    * [x] Augmented Lagrangian Method 사용 및 참고문헌(Bertsekas, 1999) 언급
    * [x] $L_c(A, \theta, \lambda)$ 식 구성 요소(Objective, Lagrange term, Penalty term) 설명
    * [x] $c \to \infty$일 때의 수렴성(Optimality condition) 설명
* **알고리즘 단계:**
    * [x] 식 (14) Primal update (argmin) 기술 및 Stochastic solver 사용 언급
    * [x] 식 (15) Dual update ($\lambda$) 기술
    * [x] 식 (16) Penalty parameter update ($c$) 기술 및 조건 설명
* **구현 세부사항:**
    * [x] 하이퍼파라미터 권장 값 ($\eta=10, \gamma=1/4$) 포함


---
title: "[Paper Review] DAG-GNN: Experiments and Conclusion"
description: "Linear/Nonlinear, Vector-valued, Discrete 데이터에 대한 광범위한 실험 결과와 논문의 최종 요약"
author: "유성현"
date: "2026-01-31"
categories: [Causal Inference, Experiments, Paper Review]
format:
  html:
    toc: true
    number-sections: false
...
본 섹션에서는 제안된 **DAG-GNN** 모델의 성능을 다양한 합성 데이터셋(Synthetic Data)과 벤치마크 데이터셋(Benchmark Data)을 통해 검증합니다. 비교 대상으로는 당시 SOTA였던 연속 최적화 기반의 **DAG-NOTEARS (Zheng et al., 2018)**를 주로 사용합니다.
...
* **Synthetic Experiments:**
    * [x] 데이터 생성 방식 (Erdos-Renyi, $n=5000$, $m \in \{10..100\}$) 기술
    * [x] Linear Case ($x=A^Tx+z$) 결과 및 NOTEARS 비교 기술
    * [x] Nonlinear Case ($h(x)=\cos(x+1)$) 식 및 FDR 개선 효과 강조
    * [x] Vector-valued Case 실험 설정 및 결과(NOTEARS의 한계) 기술
    * [x] 히트맵(Figure 4, 7) 및 그래프(Figure 2, 3, 6) Placeholder 삽입
* **Benchmark & Applications:**
    * [x] Discrete Data (Child, Alarm, Pigs) 및 GOPNILP 비교 결과 기술
    * [x] Protein Network (Sachs et al.) SHD 비교 (19 vs 22) 포함
    * [x] Knowledge Base (FB15K-237) 적용 사례 언급
* **Conclusion:**
    * [x] Generalized SEM, GNN Architecture, Polynomial Constraint 등 핵심 기여 요약
    * [x] 다양한 데이터 타입에 대한 범용성(Versatility) 강조