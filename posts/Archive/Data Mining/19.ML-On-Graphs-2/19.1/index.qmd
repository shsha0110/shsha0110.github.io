...
![Figure: 그래프 표현 학습 개요. 함수 $f(\cdot)$가 왼쪽 입력 그래프의 각 노드를 오른쪽의 저차원 임베딩 공간으로 매핑한다. 같은 색 노드(같은 군집)는 임베딩 공간에서도 근접하게 위치하며, 이를 통해 질병 간 관계와 같은 구조적 유사성이 2D 공간에 시각화된다.](./images/01_input_to_embedding.png)
...
![Figure: 그래프 표현 학습의 핵심 구성 요소. 목표는 원본 네트워크에서의 similarity(u,v)가 임베딩 내적 $\mathbf{z}_v^\top \mathbf{z}_u$와 근사되도록 인코더 ENC(u)와 ENC(v)를 학습하는 것이다. 유사도 함수는 "Need to define!"이라는 표시처럼 반드시 직접 설계해야 하는 요소이다.](./images/02_encoder_similarity.png)
...
![Figure: Shallow Encoder(임베딩 룩업)의 구조. 임베딩 행렬 Z는 '노드 수 × 임베딩 차원' 크기이며, 특정 노드의 임베딩 벡터는 해당 열을 그대로 조회하여 얻는다. 점선으로 강조된 열이 특정 노드에 해당하는 임베딩 벡터이다.](./images/03_embedding_matrix.png)
...
![Figure: GNN 아키텍처의 구조. 입력 그래프가 그래프 합성곱(Graph convolution) 레이어를 거치고, 각 레이어 사이에 Dropout 등 정규화 기법과 활성화 함수가 삽입된다. 출력은 각 노드의 임베딩 벡터이며, 이를 기반으로 노드 분류·링크 예측·그래프 분류 등 다양한 태스크를 수행할 수 있다.](./images/04_gnn_architecture.png)
...
![Figure: 현대 ML 도구 상자. 이미지(grid 구조)는 CNN으로, 텍스트·음성(sequence 구조)은 RNN으로 자연스럽게 처리된다. 이 두 구조는 모두 규칙적이고 고정된 크기를 가진다는 공통점이 있다.](./images/05_modern_ml_toolbox.png)
...
![Figure: 그래프 vs. 이미지 vs. 텍스트. 왼쪽 네트워크는 노드 수와 연결 패턴이 불규칙하여 고정 크기 입력을 가정하는 CNN/RNN을 직접 적용할 수 없다. 중앙의 이미지(격자)와 오른쪽 텍스트(선형)는 규칙적 구조를 갖는다.](./images/06_networks_vs_images.png)
...
![Figure: Naïve Approach의 한계. 그래프의 인접 행렬 A(5×5 이진 행렬)와 특징 행렬 X(2열 특징)를 연결하여 MLP에 입력한다. 입력 차원이 그래프 크기에 종속되고 노드 순서에 따라 결과가 달라지므로 범용 적용이 불가능하다.](./images/01_naive_mlp.png)
...
![Figure: GNN의 핵심 아이디어. 왼쪽은 노드 i를 중심으로 k=1, k=2 hop 이웃을 점선 원으로 표시한 계산 그래프 구조이다. 오른쪽은 집계자(aggregator₁, aggregator₂)가 계층별로 이웃 정보를 전파하고 변환하는 과정을 나타낸다.](./images/02_compute_graph_idea.png)
...
![Figure: 계산 그래프의 트리 구조. 왼쪽 입력 그래프에서 목표 노드 A를 루트로 하여 계산 트리를 전개하면, Layer-1은 직접 이웃 B·C·D를, Layer-2는 그 이웃들의 이웃까지 포함한다. 박스(□)는 집계 연산을, 점선은 집계 방향을 나타낸다.](./images/03_input_graph_only.png)

그래프 내 **모든 노드는 동시에(병렬로)** 각자의 이웃에 기반한 계산 그래프를 정의합니다.

![Figure: 모든 노드의 병렬 계산 그래프. 6개의 노드 각각이 자신을 루트로 하는 독립적인 계산 트리를 가진다. 트리의 리프 노드에는 초기 노드 특징이, 루트에는 최종 임베딩이 위치한다. 이들은 동일한 레이어 가중치를 공유하며 병렬로 처리된다.](./images/05_parallel_compute_graphs.png)
...
![Figure: 심층 GNN의 계층별 수용 범위(receptive field). Layer-0은 리프 노드의 원본 특징 $x_A, x_B, \ldots$만을 포함한다. Layer-1은 1-hop 이웃의 정보를 집계하고, Layer-2는 2-hop 이웃의 정보까지 통합한다. 레이어가 깊어질수록 각 노드 표현에 반영되는 이웃 범위가 넓어진다.](./images/06_layer_depth.png)
...
![Figure: 집계 함수의 역할. 각 박스(?)가 집계 연산을 나타낸다. 목표 노드 A의 최종 표현은 이웃들의 표현을 재귀적으로 집계하여 얻는다. 어떤 집계 함수를 선택하느냐에 따라 서로 다른 GNN 아키텍처(GCN, GraphSAGE 등)가 정의된다.](./images/07_what_is_in_box.png)
...
![Figure: GNN 레이어 구조. GNN Layer 1이 1-hop 이웃으로부터 정보를 집계하고, GNN Layer 2가 2-hop 이웃까지 추가로 집계한다. 층을 쌓을수록 각 노드 표현에 더 넓은 범위의 그래프 구조 정보가 반영된다.](./images/gnn-multilayer-hierarchy.png)
...
![Figure: GNN 레이어의 세 구성 요소 도식. 노드 v의 $l$번째 레이어 출력 $h_v^{(l)}$은 오른쪽 도식처럼 이웃 메시지 집합이 Message(1) 단계에서 변환되고 Aggregation(2) 단계에서 합산된 뒤 l-th GNN Layer를 통해 업데이트된다. 입력은 자기 자신과 이웃 노드의 (l-1)-th 표현이다.](./images/message-passing-components.png)
...
![Figure: 메시지 계산 단계. 이웃 노드들(파란색, 초록색, 빨간색)의 이전 레이어 표현에 선형 변환 $W^{(l)}$이 적용되어 메시지 벡터가 생성된다(회색 사각형). 생성된 메시지들은 이후 집계 단계의 입력으로 사용된다.](./images/gnn-step1-message-computation.png)
...
![Figure: 집계 단계. 메시지 계산이 완료된 이웃들의 메시지를 순열 불변 함수(합산·평균·최댓값 등)로 집계하여 노드 v의 중간 표현 $\hat{h}_v^{(l)}$을 생성한다. 빨간 점선 박스가 집계 연산을 나타낸다.](./images/gnn-step2-aggregation.png)
...
![Figure: Self-Connection의 역할. 노드 a를 가리키는 자기 자신으로의 화살표가 표현 보존의 역할을 상징한다. 오른쪽 도식에서 노드 v의 새로운 표현 $h_v^{(l)}$은 집계된 이웃 정보 $\hat{h}_v^{(l)}$뿐 아니라 자신의 이전 표현 $h_v^{(l-1)}$도 반영한다.](./images/gnn-step3-self-connection.png)
