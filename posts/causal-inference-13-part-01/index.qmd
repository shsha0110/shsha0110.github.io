...
* 도구변수 $\mathbf{Z}$에 의한 사영 행렬(Projection Matrix)을 **$P_Z$**라고 합시다. ($P_Z = \mathbf{Z}(\mathbf{Z}'\mathbf{Z})^{-1}\mathbf{Z}'$)
* 1단계 예측값 $\hat{\mathbf{X}}$는 $\mathbf{X}$를 투영한 것이므로 $\hat{\mathbf{X}} = P_Z\mathbf{X}$입니다.
* 외생변수 $\mathbf{W}$는 도구변수 $\mathbf{Z}$에 포함되므로 투영해도 자신이 됩니다. 즉, $P_Z\mathbf{W} = \mathbf{W}$입니다.
...
* 이제 편의상 전체 설명변수 행렬을 $\mathbf{R} = [\mathbf{X} \quad \mathbf{W}]$라 정의하고 $A$를 전개합니다. 

$$
\begin{aligned}
A &= (P_Z\mathbf{R})' (P_Z\mathbf{R}) \\
&= \mathbf{R}' P_Z' P_Z \mathbf{R} \\
&= \mathbf{R}' P_Z P_Z \mathbf{R} \quad (\because P_Z' = P_Z) \\
&= \mathbf{R}' P_Z \mathbf{R} \quad (\because P_Z P_Z = P_Z)
\end{aligned}
$$
...
* 추정량의 뒷부분인 $B = [\hat{\mathbf{X}}\; \mathbf{W}]' \mathbf{y}$ 도 동일한 방식으로 전개합니다.

$$
\begin{aligned}
B &= (P_Z\mathbf{R})' \mathbf{y} \\
&= \mathbf{R}' P_Z' \mathbf{y} \\
&= \mathbf{R}' P_Z \mathbf{y} \quad (\because P_Z' = P_Z)
\end{aligned}
$$
...
&= (\mathbf{R}' P_Z \mathbf{R})^{-1} \mathbf{R}' P_Z \mathbf{y}
\end{aligned}
$$

* 이 식은 **"원래의 변수 행렬($\mathbf{R}$ 혹은 $\mathbf{W}_{all}$)을 도구변수 공간에 투영($P_Z$)시킨 뒤, 그 투영된 변수들을 사용하여 OLS를 수행하는 것"**과 수학적으로 동일함을 보여줍니다.