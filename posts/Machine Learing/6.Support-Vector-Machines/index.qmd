...
* 소프트 마진의 쌍대 문제를 풀기 위해 **SMO 알고리즘**을 사용합니다. 이는 **좌표 상승법(Coordinate Ascent Algorithm)**에 기반을 두고 있습니다. 좌표 상승법은 한 번에 하나의 변수 $\alpha_i$만 선택하여 최적화하고 나머지는 상수로 고정하는 과정을 수렴할 때까지 반복합니다.

![Figure: 좌표 상승법(Coordinate Ascent)의 최적화 궤적. 2차원 등고선 그래프 위에서 알고리즘이 (2, -2) 지점에서 시작하여, 한 번에 하나의 축(가로 방향 또는 세로 방향)으로만 번갈아가며 직선 이동을 하여 전역 최적점(Global Maximum) 중앙을 향해 지그재그로 수렴해 가는 경로를 직관적으로 보여준다.](./images/coordinate_ascent.png)

* 하지만 SVM 쌍대 문제에는 $\sum_{i=1}^{n}\alpha_{i}y^{(i)}=0$ 이라는 제약 조건이 존재하므로, 단 하나의 $\alpha$만 변경해서는 제약 조건을 위반하게 됩니다. 
* 따라서 SMO 알고리즘은 **반드시 두 개의 $\alpha$ 변수(예: $\alpha_1, \alpha_2$)를 동시에 선택하여 최적화**합니다.

* 나머지 변수 $\alpha_3,..., \alpha_n$을 고정하면 다음의 선형 관계를 얻을 수 있습니다.

$$\alpha_{1}y^{(1)}+\alpha_{2}y^{(2)}=\zeta$$ 

* 위 식을 이용해 $\alpha_1$을 $\alpha_2$에 대한 식으로 치환하여 $W(\alpha)$에 대입하면, 단순히 1차원 2차 함수(quadratic function) 최적화 문제로 단순화됩니다.

$$\alpha_{1}=(\zeta-\alpha_{2}y^{(2)})y^{(1)}$$ 

* 이때, 새로운 $\alpha_2$는 $0 \le \alpha_i \le C$ 라는 Box 제약 조건을 만족해야 하므로, 목적 함수를 미분하여 구한 무제약 상태의 최적값 $\alpha_2^{new, unclipped}$을 구한 뒤, 허용 범위 $[L, H]$ 안으로 **클리핑(Clipping)**해 줍니다.
...
* 이러한 가벼운 2변수 최적화 과정을 KKT 조건을 모두 만족할 때까지 반복하는 것이 SMO 알고리즘의 핵심 원리입니다.