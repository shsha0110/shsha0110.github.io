...
![Figure: 선형 회귀를 단일 뉴런으로 해석한 시계열 예측 구조. 각 시점에서 동일한 입력 피처 $(x_1, x_2, x_3)$를 받는 뉴런이 독립적으로 출력(Output)을 생성합니다. 파란 시계열 위의 주황 점이 각 시점의 예측 대상이며, 모든 시점에 걸쳐 동일한 가중치 $\mathbf{w}$를 공유합니다.](./images/linear_regression_neurons.png)
...
![Figure: 선형 회귀(단일 뉴런)에서 심층 신경망(MLP)으로의 일반화. 각 시점의 입력 $(x_1, x_2, x_3)$에 대해 여러 층의 비선형 변환을 쌓아 복잡한 예측 함수 $\text{DEEP-NET}(\mathbf{x}_t)$를 구성합니다.](./images/deep_net_generalization.png)
...
![Figure: 상태 공간 모델(SSM)에서 RNN으로의 일반화 동기. SSM의 상태 $l_t$가 이전 상태 $l_{t-1}$로부터 선형 업데이트($l_t = l_{t-1} + \alpha \cdot \epsilon_t$)를 통해 전파됩니다. 각 시점에서 상태가 뉴런에 입력되고 출력(Output)이 생성되며, 이 선형 구조를 비선형으로 대체하면 RNN이 됩니다.](./images/ssm_to_rnn_motivation.png)
...
![Figure: 상태 공간 모델(SSM, 왼쪽)과 순환 신경망(RNN, 오른쪽)의 구조 비교. SSM에서는 상태 $h_t$가 선형적으로 전이되지만($l_t = l_{t-1} + \alpha \cdot \epsilon_t$), RNN에서는 비선형 활성화 함수 $\sigma$를 통해 $\mathbf{h}_t = \sigma(\theta_0 h_{t-1} + \theta_1 x_t)$로 업데이트되어 더욱 풍부한 표현을 학습할 수 있습니다.](./images/ssm_vs_rnn.png)
...
![Figure: RNN 단일 셀의 구조. 이전 은닉 상태 $h_{t-1}$ (왼쪽에서 오른쪽으로 전달되는 수평 화살표)와 현재 입력 $x_t$ (아래에서 올라오는 수직 화살표)를 받아, 활성화 함수 $\sigma$를 거쳐 새로운 은닉 상태 $h_t$를 생성합니다. 위쪽 화살표는 현재 시점의 출력 $z_t = \sigma(\theta h_t)$를 나타냅니다.](./images/rnn_single_cell.png)
...
![Figure: RNN에서의 Projection Head 적용. RNN이 전체 시계열을 처리한 뒤 생성한 여러 은닉 상태 중 마지막 상태 $z_t$ (빨간 박스로 강조)에만 projection head $y = \mathbf{w}^\top z_t$를 붙여 최종 예측값을 산출합니다.](./images/rnn_projection_head.png)
...
![Figure: 심층 신경망에서 인코더(Encoder)와 Projection Head의 역할. 녹색 입력 노드 $(x_{t,1}, x_{t,2}, x_{t,3})$로부터 파란 중간 레이어(인코더)를 거쳐 표현 $\mathbf{h}$가 생성되고, 최상단 빨간 노드 $z_t$ (Projection Head)로 변환됩니다. 확률적 예측을 위해서는 이 projection head만 수정합니다.](./images/encoder_projection_head.png)

![Figure: 확률적 예측의 두 가지 주요 방법을 보여주는 팬 차트(fan chart). 관측 구간 이후 미래 시계열에 대해 여러 신뢰 구간이 점진적으로 넓어지는 분포를 형성합니다. 빨간~주황의 진한 영역은 높은 확률의 예측 범위, 노란 외곽 영역은 낮은 확률의 꼬리 부분을 나타냅니다.](./images/probabilistic_fan_chart.png)
...
![Figure: 분위수 회귀의 예시. 관측된 시계열(검정선)에 대해 중앙값(median, 진한 초록), 50% 구간(중간 음영), 90% 구간(연한 음영)이 겹쳐 표시됩니다. 수요가 급격히 증가하는 구간에서 예측 구간의 폭이 넓어져 불확실성이 높음을 보여줍니다. P50 예측은 전체 시간의 50%에서 실제값이 그 아래에 위치하고, P90 예측은 90%에서 그 아래에 위치합니다.](./images/quantile_regression_example.png)
...
![Figure: 다중 Projection Head를 이용한 분위수 예측. 동일한 인코딩된 상태 $\mathbf{h}_t$로부터 분위수 $q \in \{0.01, 0.1, 0.3, 0.5, 0.7, 0.9, 0.99\}$에 해당하는 각기 다른 예측 궤적(곡선)이 생성됩니다. 아래쪽 곡선(파란색 0.01)이 가장 낮은 분위수, 위쪽 곡선(분홍색 0.99)이 가장 높은 분위수에 해당합니다.](./images/multiple_quantile_heads.png)
...
![Figure: 세 가지 대표적인 확률 분포의 형태 비교. 왼쪽은 다양한 $(\mu, \sigma^2)$ 조합의 가우시안(정규) 분포, 가운데는 다양한 $\lambda$ 값의 지수 분포, 오른쪽은 다양한 $\lambda$ 값의 포아송 분포. 데이터의 실제 분포 형태에 따라 적합한 분포를 선택해야 합니다.](./images/distribution_choices.png)
...
![Figure: 다양한 $(\mu, \sigma^2)$ 조합의 가우시안 분포. 예측 분포가 평균과 분산에 따라 어떻게 달라지는지 보여줍니다. 신경망은 주어진 입력에 따라 이 두 모수를 적응적으로 예측합니다.](./images/gaussian_distribution_shapes.png)
...
![Figure: 다양한 모수(n=20, p=0.25/0.5/0.75)에서의 음이항 분포 PDF. 가우시안과 달리 양수 범위에만 분포하며, 낮은 p값에서 분포가 오른쪽으로 펴지는 비대칭 형태를 보입니다. 수요량 같은 카운트(count) 데이터를 모델링하는 데 적합합니다.](./images/negative_binomial_distribution.png)
...
![Figure: 실제 판매 데이터의 분포 예시. x축은 수요량(demand/d), y축은 비율(percent)입니다. 전체 데이터의 약 98%가 0이고, 나머지 2%에 걸쳐 비율이 급격히 감소하는 롱테일(long-tail) 분포를 보입니다. 이런 데이터에는 가우시안보다 영 팽창 음이항 분포(ZINB)가 더 적합합니다.](./images/long_tail_sales_distribution.png)
...
![Figure: 우도의 직관적 이해. 두 개의 가우시안 — 주황색과 파란색 — 이 있을 때, 데이터 포인트 $\{1, 4, 5, 6, 9\}$에 대해 파란색 분포는 주황색보다 데이터 포인트들에 더 높은 확률밀도를 할당합니다. 즉, 파란색 분포의 우도가 더 높으며, MLE는 이와 같이 관측 데이터를 가장 잘 설명하는 분포를 찾는 과정입니다.](./images/likelihood_intuition.png)
...
![Figure: 사과 크기·무게와 가격의 관계를 가우시안 분포로 모델링한 예시. 왼쪽(크기 $X_1=10\text{cm}$, 무게 $X_2=10\text{g}$)의 경우 예측 평균 $\mu=1.5$, 오른쪽($X_1=15\text{cm}$, $X_2=20\text{g}$)은 $\mu=2.5$를 보여줍니다. 두 경우 모두 분포 형태는 가우시안이지만 중심 위치(평균)가 다르며, 불확실성(분산)도 입력에 따라 달라질 수 있습니다.](./images/apple_price_gaussian.png)
...
![Figure: 다양한 $\lambda$ 값에서의 지수 분포 PDF. $\lambda = 0.5$(빨간), $1.0$(초록), $1.5$(하늘) 곡선은 각각 다른 평균(2, 1, 0.67)을 가집니다. $\lambda$가 클수록 분포가 0 근방에 집중되며, 제주도 감귤처럼 낮은 가격에 집중된 데이터를 모델링하는 데 적합합니다.](./images/exponential_distribution.png)
