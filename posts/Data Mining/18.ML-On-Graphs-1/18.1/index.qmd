...
![Figure: 노드 분류(Node Classification) 개요. 왼쪽 그래프에서 회색 노드(?)는 레이블이 미지정된 상태이고, 초록·빨간 노드는 레이블이 알려진 상태입니다. 머신러닝을 거쳐 오른쪽과 같이 모든 노드에 레이블이 예측됩니다.](./images/figure_node_classification.png)
...
![Figure: 링크 예측(Link Prediction) 개요. 왼쪽에서 점선(?)으로 표시된 잠재적 엣지들이 실제 생성될 것인지를 머신러닝 모델이 판단하여, 오른쪽과 같이 체크(✓) 또는 X로 분류합니다.](./images/figure_link_prediction.png)
...
![Figure: 6개 노드(0~5)를 갖는 그래프와 이에 대응하는 $6 \times 6$ 인접 행렬. 행렬 원소 $A_{ij} = 1$이면 노드 $i$와 $j$ 사이에 엣지가 존재하며, 무방향 그래프이므로 $A = A^\top$로 대칭 행렬입니다.](./images/figure_adjacency_matrix.png)
...
![Figure: 그래프 표현 학습의 개념도. 왼쪽 원본 네트워크(original network)에서 인접한 노드 $u$와 $v$가, 오른쪽 임베딩 공간(embedding space)에서도 $\mathbf{z}_u$와 $\mathbf{z}_v$로 가깝게 위치합니다. ENC($u$)·ENC($v$)는 각 노드를 임베딩으로 변환하는 인코더 함수입니다.](./images/figure_representation_learning.png)
...
![Figure: 자카리 가라테 클럽 네트워크의 2D 노드 임베딩(Perozzi et al., 2014). 왼쪽 원본 그래프에서 각 색상은 서로 다른 커뮤니티를 나타냅니다. 오른쪽 임베딩 결과에서 같은 커뮤니티 노드들이 2D 공간에서 자연스럽게 군집화되어, 표현 학습이 그래프 구조를 효과적으로 보존함을 확인할 수 있습니다.](./images/figure_karate_club_embeddings.png)
...
![Figure: 문제 공식화 다이어그램. 원본 네트워크에서 노드 $u$는 ENC($u$)를 통해 $\mathbf{z}_u$로, 노드 $v$는 ENC($v$)를 통해 $\mathbf{z}_v$로 매핑됩니다. 학습 목표는 원본의 similarity($u$, $v$)와 임베딩 내적 $\mathbf{z}_v^\top \mathbf{z}_u$가 일치하도록 하는 것입니다.](./images/figure_problem_formulation.png)
...
![Figure: 핵심 구성요소 다이어그램. 학습 목표는 원본 네트워크의 similarity($u$, $v$)(초록 박스)가 임베딩 공간의 내적 $\mathbf{z}_v^\top \mathbf{z}_u$(파란 박스)에 근사하도록 하는 것입니다. 유사도 함수는 사전 정의(Need to define!)가 필요하며, 알고리즘마다 다르게 설정됩니다.](./images/figure_key_components.png)
...
![Figure: 계층적 소프트맥스(Hierarchical Softmax)의 이진 트리 구조. 루트 노드(Context C)에서 각 단어 노드(잎)까지의 경로를 따라 이진 분류가 반복됩니다. 각 분기의 숫자(0.57, 0.43 등)는 해당 방향으로 이동할 확률을 나타냅니다. 빨간 경로가 "I'm"에 해당하는 단어에 이르는 최적 경로 예시입니다. $N$-클래스 소프트맥스를 $O(\log N)$번의 이진 분류로 근사함으로써 계산 효율을 크게 높입니다.](./images/figure_hierarchical_softmax.png)
...
![Figure: Node2vec에서 BFS와 DFS의 탐색 방향 비교. 중심 노드 $u$에서 출발할 때, 빨간 화살표(BFS)는 $u$의 직접 이웃($s_1$, $s_2$, $s_3$)으로 향하고, 파란 화살표(DFS)는 이웃을 거쳐 더 먼 노드($s_4$, $s_5$, $s_6$, $s_7$ 등)로 뻗어 나갑니다.](./images/figure_node2vec_bfs_dfs_overview.png)
...
![Figure: 편향된 랜덤 워크의 전이 시나리오. 워커가 $s_1$에서 $w$로 이동한 상태에서, 다음 후보 노드 중 $s_2$는 $s_1$과 같은 거리에 있어 빨간색으로 표시되고, $s_3$는 더 먼 거리에 있어 갈색으로 표시됩니다. $s_1$으로의 복귀는 파란색으로 표시됩니다. 각 방향으로의 비정규화 전이 확률이 $p$·$q$에 의해 결정됩니다.](./images/figure_biased_random_walks.png)
...
![Figure: $p$·$q$ 파라미터와 전이 확률 테이블. 워커가 $s_1$에서 $w$로 이동한 상태에서, 각 후보 노드($s_1$: $1/p$, $s_2$: $1$, $s_3$: $1/q$, $s_4$: $1/q$)에 대한 비정규화 전이 확률이 할당됩니다. $s_1$과의 거리 0, 1, 2에 따라 다른 확률이 부여되며, $p$와 $q$의 상대적 크기가 BFS 대 DFS 탐색 경향을 결정합니다.](./images/figure_biased_random_walks_prob.png)
