...
* 탐색 방향 $\Delta x$를 결정한 후, 그 방향으로 얼마나 멀리 이동할지 결정하는 스텝 크기(보폭) $t$를 정하는 과정입니다. 적절한 $t$를 찾는 것은 최적화 알고리즘의 수렴 속도와 안정성에 매우 큰 영향을 미칩니다.

* 1. **Exact Line Search** 
   * 주어진 방향 $\Delta x$ 선상에서 목적 함수 $f$의 값을 완벽하게 최소화하는 최적의 보폭 $t$를 찾습니다.
   $$t = \underset{t > 0}{\text{argmin}} f(x + t \Delta x)$$ 
   * 이론적으로는 이상적이지만, 매 업데이트마다 이 최소화 문제를 정확히 풀어야 하므로 실제로는 계산 비용이 너무 높아 잘 사용되지 않습니다.

* 2. **Backtracking Line Search** 
   * 완벽한 최솟값을 찾는 대신, **'충분한 함수값 감소(Sufficient decrease)'**를 보장하는 적당한 보폭 $t$를 타협하여 찾는 실용적인 방법입니다.
   * 적당히 큰 초기 보폭에서 시작하여, 감소 조건(Armijo condition)을 만족할 때까지 보폭을 일정 비율로 줄여나갑니다(되추적).

   **[Algorithm: Backtracking Line Search]** 
   * **Given**: 현재 위치 $x$, 탐색 방향 $\Delta x$, 목적 함수 $f$, 초기 스텝 크기 $t_0$, 그리고 하이퍼파라미터 $c \in (0, 0.5), \tau \in (0, 1)$ 
   * **Set**: $t = t_0$ 
   * **Repeat**: 
     * $t = \tau t$ (조건을 만족하지 않으면 보폭을 $\tau$ 비율로 줄임) 
   * **Until**: $f(x) - f(x + t \Delta x) \ge -c t \langle \nabla f(x), \Delta x \rangle$ 

> **Note**: 위 Until 조건의 우변에서 $\langle \nabla f(x), \Delta x \rangle$는 방향 도함수에 해당하며, 하강 방향이므로 음수값을 가집니다. 즉, 실제 함수값의 감소량(좌변)이 우리가 선형 근사로 기대했던 감소량의 최소 $c$ 배 이상이 될 때까지 $t$를 조절하는 안전장치 역할을 합니다.
