---
title: "[Optimization] Lecture 3. Linear Programming (Part 2)"
...
* 데이터 포인트 군집 $V = \{v^1, \dots, v^n\} \subseteq \mathbb{R}^d$ 가 있다고 해봅시다. 우리의 목표는 이 $n$개의 벡터들로부터의 거리 $d(x)$를 최소화하는 **군집의 중심(Center)** $x$를 찾는 것입니다. 
...
> **정리 (Theorem 3.3):** 목적 함수 $f$와 제약 함수 $g_i$들이 모두 선형 표현 가능하다면, 원래의 비선형 최적화 문제는 선형 계획법으로 재표현될 수 있다.
...
### **증명 (Proof):**
* 다면체 $P = \{(x, y) : Ax + Dy \le r\}$ 에서 변수 $y_1$을 소거해 보겠습니다. 부등식 시스템 $Ax + Dy \le r$을 $y_1$의 계수 $d_{i1}$의 부호에 따라 세 그룹으로 나눕니다:
  * **Group 1 ($I_1$):** $d_{i1} > 0$ 
  * **Group 2 ($I_2$):** $d_{i1} < 0$ 
  * **Group 3 ($I_3$):** $d_{i1} = 0$ 

* 이제 $I_1$에 속하는 임의의 식 $i$와 $I_2$에 속하는 임의의 식 $j$를 짝지어(Pairing) 결합합니다. 
* $y_1$을 소거하기 위해, $I_1$의 부등식 양변에 양수인 $-d_{j1}$을 곱하고, $I_2$의 부등식 양변에 양수인 $d_{i1}$을 곱하여 두 식을 더합니다. 
* 이렇게 되면 기적처럼 $y_1$ 항의 계수가 $0$이 되어 $y_1$이 완벽히 사라진 새로운 선형 부등식이 도출됩니다. 

* 이 과정을 $I_1$과 $I_2$의 모든 조합에 대해 수행하고, $y_1$이 아예 없던 $I_3$ 그룹의 식들과 합치면 $y_1$이 투영되어 없어진 새로운 연립 부등식(다면체)을 얻습니다. 이 과정을 $y_2, \dots, y_p$에 대해 반복하면, 최종적으로 남는 식들의 집합 $proj_x(P)$ 역시 유한 개의 선형 부등식으로 정의되므로 다면체가 됨이 증명됩니다.
