...
description: "관측 데이터만으로는 옳은 정책을 학습할 수 없음을 보인 뒤, 구조적 인과 모델(SCM)과 그것이 유도하는 인과 다이어그램·마르코프 인수분해·d-분리까지 따라갑니다."
author: "유성현"
...
> **"데이터가 주는 것은 분포 $P$ 하나뿐이므로, 개입 이후의 세계 $P'$ 까지 답하려면 분포가 아니라 **현실의 모델** 인 구조적 인과 모델(SCM)이 필요합니다."**
>
> 1강이 인과 위계(PCH)라는 지도를 그렸다면, 2강은 그 지도를 지탱할 **형식 언어**를 세웁니다. 도입부의 약-생존 예시는 $P(\text{alive} \mid \text{drug}) = 1$ 이라는 흠잡을 데 없는 관측값이 정반대의 정책을 지시할 수 있음을 보이며, 어떤 알고리즘(Decision Tree, SVM, DNN)도 데이터만으로는 이 함정을 피할 수 없음을 못 박습니다.
>
> 그 빈틈을 메우는 객체가 4-튜플 $\mathcal{M} = \langle \mathbf{V}, \mathbf{U}, \mathcal{F}, P(\mathbf{U}) \rangle$ — **구조적 인과 모델(Structural Causal Model, SCM)** 이며, 이 하나의 객체가 (1) 관측 분포 $P(\mathbf{v})$, (2) **인과 다이어그램(causal graph)**, (3) 인과 위계 전체를 동시에 유도합니다. 이번 강의는 그중 1층(연관)만 파고들어, 마르코프 조건 아래에서 $P(\mathbf{v}) = \prod_i P(v_i \mid \mathbf{pa}_i)$ 라는 베이지안 인수분해를 이끌어 내고, 그래프만 보고 조건부 독립을 읽어내는 **d-분리(d-separation)** 로 마무리합니다.
...
::: {.callout-note}
## 정의 (Causal Diagram [Def. 13, Bareinboim et al., 2020])
...
$$
반면 독립을 가정한 곱은 $Y$ 를 먼저 주변화해야 하므로 $X$ 라는 조건이 사라지고, 그 자리를 $Z$ 의 **주변** 분포가 대신합니다.
$$
\begin{aligned}
P(X{=}1)\,P(Y{=}1) &= P(X{=}1) \sum_z P(Y{=}1 \mid z)\,P(z) \\[4pt]
&= P(U_X{=}1) \sum_z P(U_Y \wedge z = 1 \mid z)\,P(z) && (Y \leftarrow Z \wedge U_Y) \\[4pt]
&= P(U_X{=}1)\,P(U_Y{=}1)\,P(Z{=}1) && (z=1 \text{ 인 항만 남음}) \\[4pt]
&= P(U_X{=}1)\,P(U_Y{=}1) \sum_x P(Z{=}1 \mid x)\,P(x) \\[4pt]
&= P(U_X{=}1)\,P(U_Y{=}1)\big(P(U_X{=}0)P(U_Z{=}0) + P(U_X{=}1)\big).
\end{aligned}
$$
마지막 줄은 $Z \leftarrow X \vee \neg U_Z$ 를 $x$ 의 두 값에 대해 풀어 쓴 것입니다. $X=1$ 이면 $Z$ 는 무조건 1이고($P(Z{=}1 \mid X{=}1)=1$), $X=0$ 이면 $\neg U_Z$ 가 참일 때만 1이므로 $P(Z{=}1 \mid X{=}0) = P(U_Z{=}0)$ 입니다.

두 값을 견주어 보면, 앞의 결합 확률에는 없던 인자 $\big(P(U_X{=}0)P(U_Z{=}0) + P(U_X{=}1)\big)$ 가 곱에만 남아 있습니다. 이 인자는 $Z$ 의 주변 확률 $P(Z{=}1)$ 이며 일반적으로 1이 아니므로, 두 값이 다릅니다. 즉 $X \not\perp\!\!\!\perp Y$ 입니다. 그래프적으로는 **정보가 $X$ 에서 $Z$ 를 통해 $Y$ 로 흐릅니다.**
...
#### 2.5.6 생각해 볼 거리 (Food for Thought)
...
#### 2.5.7 구현 — 위상 정렬 (Implementation: Topological Order)

여기서부터는 이론을 코드로 옮기는 자리입니다. 먼저 DAG에서 위상 정렬을 어떻게 구현하는지 봅니다. 아이디어는 다음과 같습니다.
...
#### 2.5.8 구현 — d-분리 (Implementation: d-separation, Markovian)

이제 d-분리 판정 자체를 구현합니다. §2.5.5의 절차를 문자 그대로 옮기면 "$X$ 에서 $Y$ 로 가는 **모든 경로**를 열거"해야 하는데, 경로의 수는 노드 수에 대해 지수적으로 늘어나므로 그대로는 쓸 수 없습니다. 그래서 실제 구현은 판정의 방향을 뒤집습니다.

* **d-분리 vs. d-연결(d-connection).** d-분리를 직접 확인하는 대신, 그 부정인 **활성 경로(d-연결 경로)가 하나라도 존재하는지**를 그래프 탐색으로 찾습니다. 탐색이 $Y$ 에 도달하면 종속, 큐가 소진되면 d-분리입니다. 모든 경로를 만들어 보는 대신 노드를 방문하며 훑기 때문에 지수 폭발을 피할 수 있습니다.

유효한 d-연결 경로에 등장하는 모든 삼중쌍 $A \text{--} B \text{--} C$ 는 다음 조건을 만족해야 합니다. 이 표가 곧 §2.5.4의 활성/비활성 규칙을 "탐색이 $B$ 를 통과해 $C$ 로 나아가도 되는가"의 형태로 바꿔 쓴 것입니다.

| 삼중쌍 $A \text{--} B \text{--} C$ | $B$ 를 통과할 조건 |
|:---|:---|
| $A \to B \to C$ (연쇄) | $B \notin \mathbf{Z}$ |
| $A \leftarrow B \to C$ (공통 원인) | $B \notin \mathbf{Z}$ |
| $A \leftarrow B \leftarrow C$ (역방향 연쇄) | $B \notin \mathbf{Z}$ |
| $A \to B \leftarrow C$ (충돌점) | $B \in \mathrm{An}(\mathbf{Z})$ |

* 앞의 세 줄은 모두 $B$ 가 **비충돌점**인 경우이며, "관측하면 막힌다"는 규칙을 그대로 옮긴 것입니다.
* 마지막 줄에서 조건이 $B \in \mathbf{Z}$ 가 아니라 $B \in \mathrm{An}(\mathbf{Z})$ ($\mathbf{Z}$ 의 조상 집합) 인 점이 이 구현의 핵심입니다.

::: {.callout-tip}
## 왜 $B \in \mathbf{Z}$ 가 아니라 $B \in \mathrm{An}(\mathbf{Z})$ 인가

§2.5.3(c)에서 보았듯이, 충돌점 $B$ 자신을 관측하지 않았더라도 $\mathbf{Z}$ 안에 $B$ 의 **자손**이 하나라도 있으면 경로는 열립니다. 그런데 "$\mathbf{Z}$ 중 누군가가 $B$ 의 자손이다"는 곧 "$B$ 가 $\mathbf{Z}$ 의 조상이다", 즉 $B \in \mathrm{An}(\mathbf{Z})$ 와 같은 말입니다. 조상 집합을 한 번 계산해 두면 "충돌점 또는 그 자손이 조건에 걸렸는가"를 매번 따로 검사할 필요 없이 단일 멤버십 검사로 끝납니다. 자손을 조심하라는 경고가 구현에서는 $\mathbf{Z}$ 를 $\mathrm{An}(\mathbf{Z})$ 로 바꿔 쓰는 것 하나로 흡수되는 셈입니다.
:::

탐색은 BFS든 DFS든 상관없지만, 한 가지를 반드시 지켜야 합니다. **어느 방향으로 $B$ 에 도착했는지**($\to B$ 인지 $\leftarrow B$ 인지)를 노드와 함께 기억해야 합니다. 같은 노드라도 들어온 방향에 따라 그 노드가 충돌점이 되기도 하고 아니기도 하기 때문입니다.

```python
def d_separation(G, X, Y, Zs):
    # 시작점 X 는 들어오는 방향/나가는 방향 양쪽 모두에서 출발 가능
    q = visited_deque({('->', X), ('<-', X)})
    while q:
        direction, B = q.pop()
        if B == Y:
            return False            # 활성 경로 발견 → d-분리 아님(종속)
        if direction == '->':       # ... -> B : 화살촉이 B 로 들어옴
            if B in G.An(Zs):       # B 의 부모로 나아가면 -> B <- (충돌점)
                q.extend({('<-', C) for C in G.Pa(B)})
            if B not in Zs:         # B 의 자식으로 나아가면 -> B -> (연쇄)
                q.extend({('->', C) for C in G.Ch(B)})
        else:                       # ... <- B : 화살표가 B 에서 나옴
            if B not in Zs:         # 어느 쪽으로 가든 비충돌점
                q.extend({('->', C) for C in G.Ch(B)})
                q.extend({('<-', C) for C in G.Pa(B)})
    return True                     # 더 갈 곳이 없음 → d-분리(독립)
```

* `visited_deque` 는 이미 방문한 항목을 다시 넣지 않는 큐입니다. 방문 표시의 단위가 노드가 아니라 **(방향, 노드) 쌍**이라는 점이 중요합니다. 노드 단위로 표시하면 한쪽 방향으로 먼저 도달했다는 이유로 다른 방향의 유효한 경로를 놓치게 됩니다.
* `direction == '->'` 인 가지가 표의 네 줄을 모두 처리합니다. $B$ 의 **부모**로 이어 가면 삼중쌍이 $\to B \leftarrow$ 즉 충돌 구조가 되므로 $B \in \mathrm{An}(\mathbf{Z})$ 를 요구하고, $B$ 의 **자식**으로 이어 가면 연쇄 $\to B \to$ 가 되므로 $B \notin \mathbf{Z}$ 를 요구합니다. 두 검사가 배타적이지 않다는 점에 유의하세요 — 조건을 둘 다 만족하면 양쪽으로 모두 뻗어 나갑니다.
* `direction == '<-'` 인 가지에서는 삼중쌍이 $\leftarrow B \to$ (공통 원인) 아니면 $\leftarrow B \leftarrow$ (역방향 연쇄)뿐이라 $B$ 가 충돌점이 될 수 없습니다. 따라서 $B \notin \mathbf{Z}$ 하나만 확인하고 부모·자식 양쪽으로 확장합니다.
* 반환값의 방향이 직관과 반대라는 점도 짚어 둘 만합니다. $Y$ 에 **도달했다는 것**이 활성 경로가 있다는 뜻이므로 `False`(d-분리 아님)를 돌려주고, 끝까지 도달하지 못했을 때 비로소 `True`(d-분리)가 됩니다.

---

## 정리하며 (Closing Remarks)

이번 강의의 논증 사슬을 한 표로 압축하면 다음과 같습니다.

| 단계 | 내용 | 근거 |
|:---|:---|:---|
| **① 데이터의 한계** | $P(\text{alive} \mid \text{drug}) = 1$ 이라는 완벽한 관측이 정반대의 정책을 지시할 수 있으며, 어떤 알고리즘도 데이터만으로는 이를 교정하지 못한다 | Prima facie 예시 (§2.1) |
| **② 새 일차 객체** | 추론의 대상을 분포 $P$ 가 아니라 **현실의 모델** $\mathcal{M} = \langle \mathbf{V}, \mathbf{U}, \mathcal{F}, P(\mathbf{U}) \rangle$ 로 옮긴다 | SCM 정의 (§2.2.1) |
| **③ 모델 → 분포** | 외생 변수의 무작위성이 $\mathcal{F}$ 를 통해 전파되어 관측 분포 $P_{\mathcal{M}}(\mathbf{y}) = \sum_{\mathbf{u} \mid \mathbf{Y}(\mathbf{u}) = \mathbf{y}} P(\mathbf{u})$ 가 유도된다 | Property 1 (§2.2.2) |
| **④ 모델 → 그래프** | 함수의 인자 관계는 유향 간선으로, 공유 외생 변수는 양방향 간선으로 번역된다 (역방향 사상은 불가) | Property 2 (§2.2.3) |
| **⑤ 모델 → 위계** | 같은 $\mathcal{M}$ 이 연관·개입·반사실 세 층을 모두 함의하며, 이번 강의는 1층만 다룬다 | Property 3 (§2.2.4) |
| **⑥ 그래프 → 독립** | 위상 정렬 + 연쇄 규칙으로 $P(\mathbf{v}) = \sum_{\mathbf{u}} P(\mathbf{u}) \prod_i P(v_i \mid \mathbf{pa}_i, \mathbf{u}_i)$ 를 얻고, 여기서 $V_i \perp\!\!\!\perp \mathbf{Nd}_i \setminus \mathbf{Pa}_i \mid \mathbf{Pa}_i, \mathbf{U}_i$ 가 따라온다 | §2.3 |
| **⑦ 마르코프 인수분해** | 양방향 간선이 없으면 외생 변수가 소거되어 $P(\mathbf{v}) = \prod_i P(v_i \mid \mathbf{pa}_i)$ | §2.4 |
| **⑧ 그래프에서 읽기** | 삼중쌍의 활성/비활성 규칙을 경로 전체로 확장하면 d-분리가 되고, 이는 $\mathrm{An}(\mathbf{Z})$ 를 이용한 그래프 탐색으로 구현된다 | §2.5 |

**개인적인 감상 몇 가지를 덧붙입니다.**

* **가장 인상적인 지점**은 도입 예시가 "데이터가 부족해서" 실패하는 것이 아니라는 점입니다. $P(\text{alive} \mid \text{drug}) = 1$ 은 표본 오차도 잡음도 없는 완벽한 값이고, 표본을 무한히 늘려도 이 값은 그대로입니다. 그런데도 답이 틀립니다. **더 많은 데이터로는 결코 메울 수 없는 종류의 빈틈**이 있다는 것을 이보다 간명하게 보여 주기는 어려울 것 같습니다. 1강에서 위계가 "층이 다르다"고 선언했다면, 여기서는 그 선언이 구체적인 숫자로 시연됩니다.
* **SCM → 그래프 사상이 한 방향뿐**이라는 Remark 1은 얼핏 결함처럼 읽히지만, 사실은 이 분야 전체가 존재하는 이유입니다. 우리가 현실에서 아는 것은 함수의 **시그니처**(누가 누구의 인자인가)뿐이고 구체적 메커니즘은 모릅니다. 만약 그래프에서 SCM이 유일하게 복원된다면 인과추론은 그냥 함수 추정 문제였을 것입니다. 정보를 잃는 사상이기에, "그래프만으로 무엇을 말할 수 있는가"라는 식별(identification) 문제가 비로소 생깁니다 — 그것이 다음 회차의 주제입니다.
* **양방향 간선의 경제성**도 눈여겨볼 만합니다. 외생 변수를 그래프에서 지우되 그것이 만들던 종속성만 $\leftrightarrow$ 로 흡수해 두는 관례 덕분에, "마르코프적이다"라는 대수적 조건($\{U_i\}$ 의 결합 독립)이 **"양방향 간선이 하나도 없다"는 눈으로 확인 가능한 조건**으로 바뀝니다. 조건을 보는 데 드는 비용을 대수에서 시각으로 옮긴 설계이고, 이후 백도어·do-계산 논의가 전부 이 시각적 어휘 위에서 진행됩니다.
* **충돌점의 비대칭성**은 계속 되돌아오게 될 함정입니다. 연쇄와 공통 원인은 "관측하면 막힌다"인데 충돌점만 "관측하면 열린다"이고, 심지어 그 자손을 관측해도 열립니다. 통제 변수를 많이 넣을수록 안전하다는 회귀분석의 습관이 여기서 정면으로 부정됩니다. 구현에서 이 예외가 $\mathbf{Z}$ 를 $\mathrm{An}(\mathbf{Z})$ 로 바꿔 쓰는 것 하나로 처리된다는 점은, 이론의 예외 조항이 좋은 추상화에서 어떻게 사라지는지를 보여 주는 작은 사례이기도 합니다.
* **남은 한계**도 분명합니다. 이번 강의는 1층(연관)만 다루었으므로, 정작 우리가 알고 싶었던 "약을 모두에게 주면?"이라는 2층 질문에는 아직 형식적으로 답하지 못했습니다. 또 논의 전체가 **인과 다이어그램이 이미 주어져 있다**는 전제 위에 서 있는데, 그 그래프를 어디서 얻는가는 그 자체로 하나의 큰 주제(인과 발견)로 남아 있습니다.