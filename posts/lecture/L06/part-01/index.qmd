...
인과추론(Causal Inference)의 핵심 목표 중 하나는 관측 데이터(Observational Data)로부터 개입(Intervention)의 효과를 추정하는 것입니다. 이를 **식별(Identification)** 문제라고 합니다.

우리는 변수 $X$가 $Y$에 미치는 인과적 효과를 $P(y|do(x))$로 표기합니다. 하지만 현실 세계에서 우리는 $do(x)$(강제적 개입)가 적용된 데이터가 아니라, 자연스럽게 발생한 결합 확률 분포 $P(V)$만을 관측할 수 있습니다.

이번 포스트에서는 Judea Pearl의 Causal Calculus 프레임워크를 기반으로, **구조적 인과 모델(Semi-Markovian Models)** 하에서 $P(y|do(x))$를 계산하는 체계적인 접근법을 다룹니다. 특히 다음 세 가지 대표적인 그래프 구조를 통해 식별 가능성(Identifiability)을 분석합니다.
...
[cite_start]변수 집합 $V$와 관측되지 않은 잠재 변수(Latent Variables) 집합 $U$가 있을 때, 구조적 인과 모델 $M$은 다음과 같은 함수들의 집합으로 정의됩니다[cite: 28].

$$
v_i = f_i(pa_i, u_i), \quad \text{for each } V_i \in V
$$

여기서 $pa_i$는 $V_i$의 부모 변수(parents)를 의미합니다. [cite_start]전체 결합 확률 분포 $P(v)$는 잠재 변수 $u$에 대한 합(또는 적분)을 통해 다음과 같이 표현됩니다[cite: 29].
...
변수 $X$에 특정 값 $x$를 강제로 할당하는 개입 $do(X=x)$가 발생하면, $X$를 결정하던 기존의 구조적 함수 $X \leftarrow f_X(\dots)$는 삭제되고 상수 $X=x$로 대체됩니다. [cite_start]이를 그래프 관점에서는 $X$로 들어오는 모든 화살표를 제거하는 것으로 해석할 수 있습니다[cite: 36].

[cite_start]이때, 개입 후의 분포 $P(v'|do(x))$는 **Truncated Factorization** 공식에 의해 다음과 같이 주어집니다[cite: 17, 18].

$$
P(v'|do(x)) = \sum_{u} \prod_{V_i \in V \setminus X} P(v_i | pa_i, u_i) P(u)
$$

즉, $X$와 관련된 확률 항만 제거되고 나머지 메커니즘은 불변(Invariant)한다는 가정입니다. [cite_start]관심 있는 결과 변수 집합 $Y$에 대한 효과는 $V$에서 $X$와 $Y$를 제외한 나머지 변수들에 대해 주변화(marginalization)하여 구할 수 있습니다[cite: 21].
...
[cite_start]이 모델의 SCM은 다음과 같습니다[cite: 28].
$$
...
[cite_start]관측 분포 $P(v)$는 다음과 같이 분해됩니다[cite: 29].
$$
...
개입 $do(X=x)$가 발생하면, $X$의 결정 식은 $X=x$로 고정되고 $X$로 향하는 $Z$의 영향력은 사라집니다. [cite_start]우리는 $P(y|do(x))$를 구하고자 합니다[cite: 40].

$$
P(y|do(x)) = \sum_{u} P(z|u_z) P(y|x, z, u_y) P(u)
$$

[cite_start]여기서 잠재 변수 $u$를 $u_z, u_x, u_y$로 나누어 합을 전개하면 다음과 같습니다[cite: 42].
...
3.  **Result:**
    ...
    $$
    [cite_start]이때, $P(y|x,z) = \sum_{u_y} P(y|x,z,u_y)P(u_y)$ 이므로 최종적으로 다음과 같은 식을 얻습니다[cite: 45, 46].

$$
P(y|do(x)) = \sum_{z} P(y|x, z) P(z)
$$

이것이 바로 유명한 **Back-door Adjustment Formula**입니다. [cite_start]$Z$를 통제(conditioning)하고 $Z$의 주변 확률로 가중 평균을 냄으로써 인과 효과를 식별할 수 있습니다[cite: 47].
...
[cite_start]Bow Graph의 SCM은 다음과 같습니다[cite: 55]. 여기서 $u'$은 관측 불가능한 공통 원인입니다.
$$
\mathcal{H} = \begin{cases} 
...
[cite_start]$do(X=x)$에 대한 Truncated Factorization을 적용해 봅시다[cite: 66].

$$
P(y|do(x)) = \sum_{u} P(y|x, u', u_y) P(u)
$$

[cite_start]이 식을 전개하면 다음과 같습니다[cite: 68, 69].
...
[cite_start]Front-door 모델의 구조는 다음과 같습니다[cite: 82].
$$
...
\end{cases}
$$
[cite_start]여기서 중요한 특징은 $Z$가 $u'$의 영향을 받지 않고 오직 $x$와 $u_z$에 의해서만 결정된다는 점입니다[cite: 82].
...
[cite_start]개입 후의 분포는 다음과 같습니다[cite: 91].
$$
P(y|do(x)) = \sum_{u} P(x|u', u_x) P(z|x, u_z) P(y|z, u', u_y) P(u)
$$
[cite_start]하지만 개입 $do(X=x)$ 하에서 $P(x|u', u_x)$ 항은 삭제되거나 1이 되므로(intervention에 의해 고정됨), 식은 다음과 같이 정리됩니다[cite: 95].
...
[cite_start]잠재 변수들을 그룹화하여 전개합니다[cite: 96].
$$
...
[cite_start]내부의 $u_y$에 대한 합을 계산하면 $P(y|z, u')$이 됩니다[cite: 97, 98].
$$
...
$X$가 $x'$일 때 $Z$와 $Y$ 사이의 관계를 이용해 $u'$ 항을 $x'$에 대한 식으로 변환할 수 있습니다. [cite_start]유도 과정을 따라가면 다음과 같은 최종 식을 얻습니다 [cite: 130-134].
...
[cite_start]결과적으로 다음과 같은 식별 공식을 얻습니다[cite: 134].
