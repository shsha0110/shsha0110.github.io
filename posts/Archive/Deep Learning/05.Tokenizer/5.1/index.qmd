...
description: "단어 사전의 OoV 문제와 문자 단위 토큰화의 비효율 사이에서 BPE가 어떻게 절충점을 찾는지, 그리고 정수 토큰이 어떻게 의미를 담은 벡터가 되는지를 밑바닥부터 구현하며 따라갑니다."
author: "유성현"
...
> 본 포스트는 서울대학교 컴퓨터공학부 **Hyun Oh Song** 교수님의 *Introduction to Deep Learning* (M2177.0043) 5강 실습 세션(*Tokenizer*)의 노트북을 바탕으로, 코드와 그 배경이 되는 논리를 함께 따라갈 수 있도록 재구성한 것입니다. BPE 의사코드와 서브워드 분해 예시 그림은 강의에서 제공한 자료입니다.

## 한 줄 요약 (TL;DR)

> **"토큰화는 어휘 사전의 크기와 표현력 사이의 절충 문제이고, BPE는 그 절충을 데이터로부터 학습합니다."**
>
> 신경망은 텍스트를 직접 다루지 못하므로 문자열을 정수 열로 바꾸는 관문이 필요합니다. 가장 소박한 방법은 코퍼스의 단어를 그대로 사전에 담는 것이지만, 학습 때 보지 못한 단어가 들어오면 표현할 방법이 없다는 **OoV 문제**에 부딪힙니다. 반대 극단인 **문자 단위 토큰화**는 어떤 텍스트도 처리하지만 토큰 하나에 의미가 담기지 않고 시퀀스가 지나치게 길어집니다. **BPE**는 문자 단위 어휘에서 출발해 자주 함께 등장하는 쌍을 반복해서 병합함으로써 이 둘 사이의 절충점을 데이터로부터 찾아내고, 학습된 병합 규칙을 순서대로 적용하는 것만으로 처음 보는 단어도 익숙한 서브워드로 분해합니다. 후반부는 그렇게 얻은 정수 토큰을 벡터로 바꾸는 문제를 다룹니다. 원-핫 인코딩은 차원이 어휘 크기만큼 커지고 모든 단어가 서로 등거리라는 두 가지 결함이 있으며, 이를 **임베딩 레이어**(사실상 조회 테이블)로 대체하면 학습된 벡터 공간 안에서 "king − man + woman ≈ queen" 같은 의미 관계가 방향 벡터로 나타납니다.

> **실습 환경.** 본 실습은 Google Colab 또는 로컬 환경에서 진행한다고 가정합니다. Colab 사용 시 아래와 같이 드라이브를 마운트하고 작업 디렉터리를 옮깁니다.
>
> ```python
> try:
>     import os
...
![Figure 5: GloVe 임베딩을 2차원으로 무작위 투영한 산점도입니다. 오른쪽 위의 'king'에서 왼쪽 아래의 'queen'으로 향하는 긴 화살표와, 그 아래 'man'에서 'woman'으로 향하는 짧은 화살표가 그려져 있습니다. 두 화살표의 **방향이 거의 나란하다**는 점이 핵심입니다. 즉 "남성 → 여성"이라는 의미 변화가 임베딩 공간에서 특정한 방향 벡터로 표현되며, 그 방향은 어떤 단어 쌍에 적용하든 대체로 일정합니다. 이것이 $\text{vector('king')} - \text{vector('man')} + \text{vector('woman')} \approx \text{vector('queen')}$ 이라는 선형 관계가 성립하는 기하학적 이유입니다. 화살표의 길이가 서로 다른 것은 2차원 무작위 투영 과정에서 생긴 왜곡입니다.](./images/figure05_embedding_king_queen.png)
...
![Figure 6: 여섯 개의 나라(korea, russia, france, uk, japan, china)와 그 수도(seoul, moscow, paris, london, tokyo, beijing)를 2차원 평면에 찍고, 각 나라에서 해당 수도로 화살표를 그은 산점도입니다. 여섯 화살표가 완전히 같지는 않지만 대체로 **왼쪽 위를 향하는 비슷한 방향**으로 모입니다. 앞의 성별 관계와 마찬가지로 "~의 수도"라는 관계도 임베딩 공간에서 하나의 방향 벡터로 부호화되어 있다는 뜻이며, 학습 과정에서 명시적으로 가르친 적이 없는 관계가 단어들의 공기(co-occurrence) 통계만으로 기하 구조에 새겨졌음을 보여 줍니다.](./images/figure06_embedding_countries_capitals.png)
...
![Figure 7: 위 코드의 결과입니다. 여섯 수도를 평면에 찍은 뒤, 오른쪽 아래의 'russia'에서 "수도 방향 벡터" $(\text{london} - \text{uk})$ 만큼 이동한 지점을 화살표 끝으로 표시하고 그 주위에 원을 그렸습니다. 화살표가 도착한 지점이 'moscow' 바로 곁이라는 점이 핵심입니다. 즉 한 나라에서 추출한 관계 벡터를 전혀 다른 나라에 적용해도 올바른 수도 근처로 이동하며, 이는 관계가 특정 단어 쌍에 묶인 것이 아니라 **공간 전체에 걸쳐 일관된 방향**임을 뜻합니다. 다만 2차원 투영이라 'beijing'도 원 안에 가깝게 놓이므로, 다음 절에서는 원래의 50차원 공간에서 코사인 유사도로 다시 확인합니다.](./images/figure07_embedding_capital_direction.png)
...
| 단계 | 내용 | 근거 |
|:---|:---|:---|
| **① 문제 설정** | 언어 모델은 다음 토큰을 예측하는 신경망이므로, 텍스트를 정수 열로 바꾸고 되돌리는 양방향 관문이 필요하다 | `encode` / `decode` 규약 |
| **② 단어 사전의 한계** | 코퍼스의 단어를 그대로 사전에 담으면, 학습 때 보지 못한 단어는 표현할 수단이 아예 없다 | OoV(Out-of-Vocabulary) 문제 |
| **③ 반대 극단의 한계** | 문자 단위로 쪼개면 OoV는 사라지지만 토큰 하나가 의미를 담지 못하고 시퀀스가 지나치게 길어져 계산이 비효율적이 된다 | 문자 수준 토큰화 |
| **④ BPE의 절충** | 문자 어휘에서 출발해 **가장 빈번한 인접 쌍을 반복 병합**하며 상향식으로 어휘를 키운다. 병합 횟수가 곧 어휘 크기와 표현력의 손잡이가 된다 | BPE 학습 의사코드 |
| **⑤ 인코딩의 일관성** | 인코딩 시에는 학습 때와 **같은 순서로** 병합 규칙을 적용해야 한다. 순서가 곧 규칙의 우선순위이기 때문이다 | BPE 인코딩 의사코드 |
| **⑥ OoV의 해소** | 처음 보는 "lowest"도 `l o w e s t` → `low est` 처럼 익숙한 서브워드로 분해되므로, 미등록 단어를 만나도 표현이 끊기지 않는다 | 서브워드 분해 예시 |
| **⑦ 정수를 벡터로** | 원-핫 인코딩은 차원이 어휘 크기와 같고 모든 단어가 서로 등거리라 의미를 담을 수 없다 | 원-핫의 두 결함 |
| **⑧ 임베딩 레이어** | 원-핫 벡터와 가중치 행렬의 곱은 결국 **한 열을 조회하는 연산**이므로, 곱셈 대신 조회 테이블로 구현하면 같은 결과를 훨씬 싸게 얻는다 | 원-핫 곱 = 열 추출 |
| **⑨ 학습된 공간의 구조** | GloVe 임베딩에서 "남성 → 여성", "나라 → 수도" 같은 관계가 일관된 **방향 벡터**로 나타나며, 코사인 유사도로도 `king - man + woman` 의 최근접 단어가 `queen` 임이 확인된다 | 선형 연산 예시 · 코사인 유사도 |
