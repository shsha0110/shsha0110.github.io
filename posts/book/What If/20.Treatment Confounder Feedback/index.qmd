...
![Figure 20.1: 치료-교란요인 피드백이 존재하는 DAG](./images/figure_20_1_feedback.png)
> **Figure 20.1 해석**:
...
![Figure 20.2: 시변 교란은 있지만 피드백은 없는 DAG](./images/figure_20_2_no_feedback.png)
> **Figure 20.2 해석**:
...
![Figure 20.3: Sharp Null 하에서의 Treatment-Confounder Feedback](./images/figure_20_3_simplified_null.png)
> **Figure 20.3 상세 설명**:
...
![Figure 20.5: Causal diagram illustrating the collider bias. $U_1$ is an unmeasured variable affecting both $L_1$ and $Y$. $A_0$ affects $L_1$. Conditioning on the collider $L_1$ (indicated by a square box) opens the path $A_0 \rightarrow L_1 \leftarrow U_1 \rightarrow Y$, inducing a non-causal association between prior treatment $A_0$ and outcome $Y$.](./images/collider_bias_structure.png)
...
![Figure 20.5: Causal diagram representing treatment-confounder feedback. A0 affects L1, and L1 affects A1. U1 is an unmeasured common cause of L1 and Y.](./images/dag_feedback_structure.png)
...
![Figure 20.7: Scenario where L1 is on the causal pathway from A0 to Y and acts as a confounder for A1.](./images/dag_causal_pathway.png)
...
![Figure 20.6: Observational study setting where conditioning on L1 opens the path A0 <-> W0 -> L1 <-> U1 -> Y.](./images/dag_observational.png)
...
![Figure: Treatment-Confounder Feedback이 존재하는 상황에서의 인과 다이어그램 (Figure 20.8 & 20.9). $A_0$가 $A_1$에 직접적인 영향을 미치는 상황을 묘사한다. Figure 20.8(왼쪽)은 $U_1$이 $L_1$과 $Y$의 공통 원인인 경우를, Figure 20.9(오른쪽)는 $W_0$가 $A_0$와 $L_1$의 공통 원인인 경우를 나타낸다. 두 경우 모두 $L_1$을 조건부로 통제하더라도 $A_1$과 $Y$ 사이의 뒷문 경로(Backdoor path)가 열려 있음을 보여준다.](./images/figure_20_8_and_20_9.png)
