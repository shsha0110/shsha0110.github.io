...
description: "연속 상태·행동 공간의 제어 — LQR의 리카티 방정식, 비선형을 국소 선형화하는 DDP, 칼만 필터를 결합한 LQG, 그리고 모델 없이 정책을 직접 미분하는 Policy Gradient."
author: "유성현"
...
> 본 포스트는 서울대학교 컴퓨터공학부 · 인공지능대학원(IPAI) **박재식(Jaesik Park)** 교수님의 *Introduction to Machine Learning* (4190.428) 강의 자료(18강 *Reinforcement Learning, Part 2 — LQR, DDP, LQG*)를 바탕으로, 강의를 처음 듣는 독자도 논리적 흐름을 따라갈 수 있도록 재구성한 것입니다. 원 강의 자료는 MIT 6.036(2019 Fall)과 Stanford CS229(2023–2024 Summer)의 강의 노트를 상당 부분 차용하고 있습니다. 17강의 이산 MDP와 가치 반복에서 이어집니다.

## 한 줄 요약 (TL;DR)

> **"연속 상태에서는 모든 상태를 훑는 것이 불가능합니다. 그래서 이번 회차의 네 기법은 전부 '문제를 풀 수 있는 모양으로 바꾸는' 전략입니다 — 가정을 좁히거나(LQR), 국소 근사하거나(DDP), 추정과 제어를 분리하거나(LQG), 아예 모델을 포기하거나(Policy Gradient)."**
>
> 출발점은 **유한 시계 MDP**입니다. 보상과 전이가 시간에 따라 달라질 수 있으므로 최적 정책도 **시간에 의존**하게 되고(비정상 정책), 마지막 시점에서 거꾸로 내려오는 **후방 귀납**이 자연스러운 해법이 됩니다.
>
> **LQR**은 여기에 강한 가정을 얹습니다 — 동역학이 선형이고 보상이 이차식이면, 최적 가치함수가 상태의 이차형식으로 유지되고 최적 행동이 상태의 **선형 함수** $a_t = L_t s_t$로 떨어집니다. 계수는 **이산 리카티 방정식**을 후방으로 풀어 얻으며, 놀랍게도 이 해는 **프로세스 노이즈의 공분산에 전혀 의존하지 않습니다.**
>
> **DDP**는 그 가정이 깨질 때의 대응입니다. 현재 궤적 주변에서 동역학을 1차, 보상을 2차로 테일러 전개하면 국소적으로 LQR 문제가 되고, 그 해로 새 궤적을 만들어 다시 전개하기를 반복합니다.
>
> **LQG**는 상태를 직접 볼 수 없는 경우입니다. **칼만 필터**로 관측에서 상태 분포를 추정한 뒤 그 추정치에 LQR을 적용하는데, 이 분리가 정당한 이유가 바로 위의 **노이즈 독립성**입니다.
>
> 마지막 **Policy Gradient**는 방향을 완전히 바꿉니다. 가치함수를 거치지 않고 $\eta(\theta)$를 $\theta$로 직접 미분하는데, 궤적 분포가 $\theta$에 의존해 재매개변수화 트릭을 쓸 수 없습니다. **로그 미분 트릭**이 이를 뚫으며, 그 결과 식에서 **전이 확률이 완전히 사라져** 모델을 몰라도 학습할 수 있게 됩니다. 남는 문제인 분산은 **베이스라인**을 빼서 다스립니다.
...
![Figure 2: 베이스라인을 사용한 바닐라 정책 경사 알고리즘(Algorithm 11). 매 반복마다 (1) 현재 정책을 실행해 궤적 집합을 수집하고 미래 누적 보상 $R_{\ge t} = \sum_{j \ge t}\gamma^{j-t}R(s_j,a_j)$를 계산한다. (2) $\sum_\tau \sum_t (R_{\ge t} - B(s_t))^2$을 최소화하여 베이스라인 함수 $B$를 적합(fit)한다. 즉 베이스라인을 미래 보상의 회귀 예측치로 학습한다. (3) 경사 추정량 $\sum_\tau \sum_t \nabla_\theta \log \pi_\theta(a_t|s_t) \cdot (R_{\ge t} - B(s_t))$로 정책 파라미터 $\theta$를 갱신한다. 이 그림은 'advantage($R_{\ge t} - B(s_t)$)에 따라 행동 확률을 높이거나 낮춘다'는 정책 경사의 핵심 루프를 보여준다.](./images/algorithm11_policy_gradient.png)
...
| 단계 | 내용 | 근거 |
|:---|:---|:---|
| **① 토대** | 보상·전이가 시간에 의존하면 최적 정책도 시간에 의존한다 (비정상 정책) | §1.2–§1.3 |
| **② 해법 구조** | 마지막 시점에서 거꾸로 내려오는 후방 귀납 동적 계획법 | §1.4 |
| **③ 가정 1** | 동역학이 선형: $s_{t+1} = A_t s_t + B_t a_t + w_t$ | §2.1 |
| **④ 가정 2** | 보상이 이차식(음의 준정부호): $-s^\top U s - a^\top W a$ | §2.1 |
| **⑤ 결과 1** | 최적 가치함수가 상태의 이차형식 $s^\top \Phi_t s + \Psi_t$로 **닫혀 있다** | §2.4 |
| **⑥ 결과 2** | 최적 행동이 상태의 선형 함수 $a_t = L_t s_t$가 된다 | §2.4 |
| **⑦ 계산** | 계수 $\Phi_t, \Psi_t$는 이산 리카티 방정식을 후방으로 풀어 얻는다 | §2.4 |
| **⑧ 핵심 성질** | $L_t$가 프로세스 노이즈의 공분산 $\Sigma_w$에 의존하지 않는다 | §2.4 |
| **⑨ 가정이 깨질 때** | 비선형 동역학은 현재 궤적 주변에서 테일러 전개로 선형화한다 | §3 |
| **⑩ DDP** | 선형화 → LQR로 풀기 → 새 궤적 생성 → 재선형화를 반복한다 | §4 |
| **⑪ 상태를 못 볼 때** | 관측만 주어지는 POMDP에서는 신념 상태를 다뤄야 한다 | §5 |
| **⑫ LQG** | 칼만 필터로 상태 분포를 추정하고, 그 추정치에 LQR을 적용한다 | §6 |
| **⑬ 분리의 근거** | ⑧의 노이즈 독립성이 "추정과 제어를 따로 해도 된다"를 정당화한다 | §6.3 |
| **⑭ 관점 전환** | 가치함수를 거치지 않고 정책 $\pi_\theta$를 직접 미분한다 | §7.1–§7.2 |
| **⑮ 난관** | 궤적 분포가 $\theta$에 의존해 재매개변수화 트릭을 쓸 수 없다 | §7.3 |
| **⑯ 돌파** | 로그 미분 트릭 $\nabla_\theta P = P\,\nabla_\theta \log P$로 기댓값 안으로 들어간다 | §7.4 |
| **⑰ 수확** | 전이 확률이 상쇄되어 사라진다 → 모델 없이(model-free) 학습 가능 | §7.6 |
| **⑱ 해석** | 미래 보상의 부호에 따라 그 행동의 확률을 높이거나 낮춘다 | §7.7 |
| **⑲ 마무리** | 베이스라인을 빼면 편향 없이 분산만 줄어든다 (거의 최적은 $V^{\pi_\theta}$) | §7.8–§7.9 |
