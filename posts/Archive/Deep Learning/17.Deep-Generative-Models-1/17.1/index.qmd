...
description: "오토인코더가 왜 생성 모델이 되지 못하는지에서 출발해, VAE를 이해하는 데 필요한 세 재료 — KL 발산·젠센 부등식·가우시안 KL — 을 갖추고 ELBO를 예고합니다."
author: "유성현"
...
> 본 포스트는 서울대학교 컴퓨터공학부 **Hyun Oh Song** 교수님의 *Introduction to Deep Learning* (M2177.0043) 강의 자료(17강 *Deep Generative Models I: Autoencoders and the VAE Toolkit*)를 바탕으로, 강의를 처음 듣는 독자도 논리적 흐름을 따라갈 수 있도록 재구성한 것입니다. 이 강의는 18강의 VAE로 이어지는 2회차 로드맵의 전반부에 해당합니다.

## 한 줄 요약 (TL;DR)

> **"오토인코더는 표현은 배우지만 표본을 만들지는 못합니다. 그 간극을 메우려면 KL 발산 · 젠센 부등식 · 가우시안 KL 세 가지 도구가 먼저 필요합니다."**
>
> 이번 강의는 VAE를 곧바로 소개하는 대신, VAE가 왜 필요한지와 그것을 이해하는 데 무엇이 필요한지를 차례로 깝니다. 출발점은 **오토인코더**입니다. 입력을 좁은 코드 $z$ 로 압축했다가 되살리도록 학습하면 유용한 표현을 얻지만, 학습이 끝난 뒤 인코더를 버리고 $z$ 를 아무렇게나 뽑아 디코더에 넣으면 그럴듯한 표본이 나오지 않습니다. 인코딩된 학습 데이터가 잠재 공간의 복잡한 부분집합만 차지해 그 사이에 **구멍(hole)** 이 생기기 때문이며, 애초에 오토인코더는 $z$ 를 뽑을 **분포를 정의하지 않습니다.** 이 결함을 메우려면 잠재 변수에 사전 분포 $p(z) = \mathcal{N}(0, I)$ 를 두고 디코더를 확률 분포 $p_\theta(x \mid z)$ 로 보는 관점이 필요한데, 여기서 주변 우도 $p_\theta(x) = \int p_\theta(x \mid z)p(z)\,dz$ 가 계산 불가능하다는 문제가 나타납니다. 강의의 나머지는 이 적분을 우회할 하한(ELBO)을 세우기 위한 재료를 갖추는 데 쓰입니다. 두 분포의 어긋남을 재는 **KL 발산**(거리가 아니며 방향이 중요하다는 점 포함), 기댓값과 함수 적용 순서를 바꿔 하한을 만들어 주는 **젠센 부등식**, 그리고 인코더와 사전 분포가 모두 가우시안일 때 KL이 닫힌 형태로 떨어지는 **가우시안 KL** 입니다.

## 1. 비지도 학습과 생성 모델링 (Unsupervised Learning and Generative Modeling)
...
## 2. 오토인코더: 표현 학습 (Autoencoders: Representation Learning)
...
## 3. 확률적 잠재 변수 표기법 (Probabilistic Latent-Variable Notation)
...
## 8. 부록: 그 밖의 발산들 (Backup: Extra Divergences)
...
| 단계 | 내용 | 근거 |
|:---|:---|:---|
| **① 목표 설정** | 생성 모델의 목표는 데이터 $x^{(1)}, \dots, x^{(N)} \sim p_{\text{data}}(x)$ 에 높은 확률을 부여하는 $p_\theta(x)$ 를 배우고, 그로부터 새 표본을 뽑는 것이다 | 생성 모델링의 목적 |
| **② 오토인코더의 성취** | 인코더 $f_\phi$ 와 디코더 $g_\theta$ 를 복원 손실로 학습하면 병목 $z$ 에 데이터의 구조가 압축된다 | 표현 학습 |
| **③ 오토인코더의 한계** | 학습 후 $z$ 를 아무렇게나 뽑으면 그럴듯한 표본이 나오지 않는다. 인코딩된 데이터가 잠재 공간의 복잡한 부분집합만 차지해 **구멍**이 생기고, 애초에 $z$ 를 뽑을 분포가 정의되어 있지 않다 | latent space의 hole |
| **④ 확률적 재해석** | 복원 손실은 사실 디코더 우도의 음의 로그다. 가우시안 디코더면 제곱 오차, 베르누이 디코더면 교차 엔트로피가 된다 | $-\log p_\theta(x \mid z)$ |
| **⑤ 잠재 변수 모델** | 사전 분포 $p(z) = \mathcal{N}(0, I)$ 를 두면 $z$ 를 뽑을 곳이 생긴다. 그러나 주변 우도 $p_\theta(x) = \int p_\theta(x \mid z) p(z)\, dz$ 는 계산 불가능하다 | 적분의 처리 불가능성 |
| **⑥ 재료 ①: KL 발산** | 두 분포의 어긋남을 재는 $D_{\mathrm{KL}}(p \Vert q)$ 는 대칭이 아니어서 **방향이 의미를 바꾸고**, $p$ 가 양의 확률을 주는 곳에서 $q$ 가 0이면 발산한다 | 정의 1 · 2 · 지지 조건 |
| **⑦ 재료 ②: 젠센 부등식** | $\log$ 는 오목하므로 $\mathbb{E}[\log X] \leq \log \mathbb{E}[X]$ 다. 이 한 줄이 $\log p_\theta(x)$ 의 **하한**을 만들어 주고, 동시에 $D_{\mathrm{KL}} \geq 0$ 도 증명한다 | 정의 3 · 정리 5 |
| **⑧ 재료 ③: 가우시안 KL** | $q_\phi(z \mid x) = \mathcal{N}(\mu_\phi, \mathrm{diag}(\sigma^2_\phi))$ 와 $p(z) = \mathcal{N}(0, I)$ 사이의 KL은 $\frac{1}{2}\sum_j (\mu_j^2 + \sigma_j^2 - 1 - \log \sigma_j^2)$ 로 닫힌 형태다. $\mu_j = 0, \sigma_j = 1$ 에서 0이 된다 | 가우시안 KL · 검산 |
| **⑨ 도달점: ELBO** | 세 재료를 합치면 $\log p_\theta(x) = \mathcal{L}(x; \theta, \phi) + D_{\mathrm{KL}}(q_\phi(z \mid x) \Vert p_\theta(z \mid x))$ 이고, KL이 음이 아니므로 $\mathcal{L}$ 은 하한이다. ELBO는 "복원 품질 − 사전 분포로부터의 이탈"로 읽힌다 | ELBO 미리보기 |
| **⑩ 부록** | 순방향 KL은 평균을 좇고(mean-seeking) 역방향 KL은 최빈값을 좇는다(mode-seeking). JSD는 대칭화된 변형이고, $f$-발산은 이들을 하나의 틀로 묶는다 | 부록: 그 밖의 발산들 |
