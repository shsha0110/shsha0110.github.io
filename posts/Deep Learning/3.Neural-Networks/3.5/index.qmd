...
Dropout을 코드로 구현하는 방법에는 크게 두 가지가 있습니다. 오늘날 대부분의 딥러닝 프레임워크는 효율성을 위해 두 번째 방법(Inverted Dropout)을 채택합니다.
...
def train_step(X):
    ...
    U1 = np.random.rand(*H1.shape) < p   # Dropout 마스크 생성
    H1 *= U1                             # 무작위로 끄기
    # ... 생략 ...

def predict(X):
    H1 = np.maximum(0, np.dot(W1, X) + b1) * p  # Test 타임에 p를 곱해 스케일링
    # ... 생략 ...

```

이 방식은 추론을 담당하는 `predict` 함수의 코드를 수정해야 한다는 단점이 있습니다. 시스템의 유지 보수와 배포 관점에서 테스트 코드는 가급적 건드리지 않는 것이 좋습니다.
...
def train_step(X):
    H1 = np.maximum(0, np.dot(W1, X) + b1)
    # 훈련 단계에서 미리 / p 로 나누어 스케일링을 수행!
    U1 = (np.random.rand(*H1.shape) < p) / p 
    H1 *= U1
    # ... 생략 ...

def predict(X):
    H1 = np.maximum(0, np.dot(W1, X) + b1)  # Test 코드는 아무런 수정도 필요 없음!
    # ... 생략 ...

```

**Inverted Dropout**은 Test 시에 곱해야 할 $p$를, 아예 Train 시 마스크 자체를 $p$로 나누어버림으로써 미리 보상(Compensate)해버리는 방식입니다. 이로 인해 Test 단계의 네트워크가 완벽하게 불변(unchanged)인 상태로 유지되어 추론 코드가 훨씬 깔끔해집니다.
